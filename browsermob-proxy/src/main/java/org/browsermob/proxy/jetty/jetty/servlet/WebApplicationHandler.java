...
/* --------------------------------------------------------------------- */
/**
 * WebApp HttpHandler. This handler extends the ServletHandler with security,
 * filter and resource capabilities to provide full J2EE web container support.
 * <p>
 * 
 * @since Jetty 4.1
 * @see org.browsermob.proxy.jetty.jetty.servlet.WebApplicationContext
 * @version $Id: WebApplicationHandler.java,v 1.62 2006/01/04 13:55:31
 *          gregwilkins Exp $
 * @author Greg Wilkins
 */
public class WebApplicationHandler extends ServletHandler {
	...
	/** The _servlet filter map. */
	private MultiMap _servletFilterMap = new MultiMap();

	/** The _accept ranges. */
	private boolean _acceptRanges = true;

	/** The _filter chains cached. */
	private boolean _filterChainsCached = true;

	/** The _web application context. */
	...
	/** The _context attribute listeners. */
	protected transient Object _contextAttributeListeners;

	/** The jsr154 filter holder. */
	...
	/** The _named chain cache. */
	protected transient HashMap _namedChainCache[];

	/* ------------------------------------------------------------ */
	/**
	 * Checks if is accept ranges.
	 * 
	 * @return true, if is accept ranges
	 */
	 ...
	 * @param filterName
	 *            The name of the filter (must already be added or defined)
	 * @param dispatches
	 *            An integer formed by the logical OR of FilterHolder.__REQUEST,
	 *            FilterHolder.__FORWARD,FilterHolder.__INCLUDE and/or
	 *            FilterHolder.__ERROR.
	 * @return The holder of the filter instance.
	 */
	public FilterHolder addFilterPathMapping(String pathSpec,
			String filterName, int dispatches) {
	 ...
	 * 
	 * @param servletName
	 *            The name of the servlet to be filtered.
	 * @param filterName
	...
	public FilterHolder addFilterServletMapping(String servletName,
		...
		if (holder == null)
			throw new IllegalArgumentException("Unknown filter :" + filterName);
		_servletFilterMap.add(servletName, new FilterMapping(null, holder,
				dispatches));
		return holder;
	}
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.util.Container#addEventListener(java.util.
	 * EventListener)
	 */
	public synchronized void addEventListener(EventListener listener)
		...
		if (listener instanceof ServletRequestAttributeListener) {
			_requestAttributeListeners = LazyList.add(
					_requestAttributeListeners, listener);
		}

		if (listener instanceof ServletContextAttributeListener) {
			_contextAttributeListeners = LazyList.add(
					_contextAttributeListeners, listener);
		}
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.util.Container#removeEventListener(java.util
	 * .EventListener)
	 */
	public synchronized void removeEventListener(EventListener listener) {
		...
		_requestAttributeListeners = LazyList.remove(
				_requestAttributeListeners, listener);
		_contextAttributeListeners = LazyList.remove(
				_contextAttributeListeners, listener);
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#setSessionManager
		...
		if (getHttpContext() != null) {
			...
			if (old != null && old != sm) {
				if (_sessionListeners != null) {
					for (Iterator i = LazyList.iterator(_sessionListeners); i
							.hasNext();) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#doStart()
	 */
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#initializeServlets
	 * ()
	 */
	public void initializeServlets() throws Exception {
		...
		if (LazyList.size(_requestAttributeListeners) > 0
				|| LazyList.size(_requestListeners) > 0) {

			if (jsr154Filter == null)
				log.warn("Filter jsr154 not defined for RequestAttributeListeners");
			else {
				jsr154Filter
						.setRequestAttributeListeners(_requestAttributeListeners);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#doStop()
	 */
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#getErrorPage(int,
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHttpRequest)
	 */
	public String getErrorPage(int status, ServletHttpRequest request) {
		String error_page = null;
		Class exClass = (Class) request
				.getAttribute(ServletHandler.__J_S_ERROR_EXCEPTION_TYPE);
		...
		if (ServletException.class.equals(exClass)) {
			...
			if (error_page == null) {
				Throwable th = (Throwable) request
						.getAttribute(ServletHandler.__J_S_ERROR_EXCEPTION);
		...
		if (error_page == null && exClass != null) {
			while (error_page == null && exClass != null
					&& _webApplicationContext != null) {
				error_page = _webApplicationContext.getErrorPage(exClass
						.getName());
		...
		if (error_page == null && _webApplicationContext != null)
			error_page = _webApplicationContext.getErrorPage(TypeUtil
					.toString(status));
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#dispatch(java
	 * .lang.String, javax.servlet.http.HttpServletRequest,
	 * javax.servlet.http.HttpServletResponse,
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHolder, int)
	 */
	protected void dispatch(String pathInContext, HttpServletRequest request,
		...
		if (type == Dispatcher.__REQUEST) {
			...
			if (StringUtil.startsWithIgnoreCase(pathInContext, "/web-inf")
					|| StringUtil.startsWithIgnoreCase(pathInContext,
							"/meta-inf")) {
			...
			// Security Check
			if (!getHttpContext().checkSecurityConstraints(pathInContext,
			...
			if (context != null
					...
					&& pathInContext != null
					&& pathInContext
							.endsWith(FormAuthenticator.__J_SECURITY_CHECK)) {
				ServletHttpRequest servletHttpRequest = (ServletHttpRequest) context
						.getHttpConnection().getRequest().getWrapper();
				ServletHttpResponse servletHttpResponse = servletHttpRequest
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the chain for name.
	 * 
	 * @param requestType
	 *            the request type
	 * @param servletHolder
	 *            the servlet holder
	 * @return the chain for name
	 */
	private FilterChain getChainForName(int requestType,
			ServletHolder servletHolder) {
		if (servletHolder == null) {
			throw new IllegalStateException(
					"Named dispatch must be to an explicitly named servlet");
		}

		if (_filterChainsCached) {
			synchronized (this) {
				if (_namedChainCache[requestType].containsKey(servletHolder
						.getName()))
					return (FilterChain) _namedChainCache[requestType]
							.get(servletHolder.getName());
			}
		}
		...
		if (_filterChainsCached) {
			synchronized (this) {
				...
				_namedChainCache[requestType].put(servletHolder.getName(),
						chain);
			}
	 ...
	 * @param servletHolder
	 *            the servlet holder
	 * @return the chain for path
	 */
	private FilterChain getChainForPath(int requestType, String pathInContext,
			ServletHolder servletHolder) {
		if (_filterChainsCached) {
			synchronized (this) {
				if (_chainCache[requestType].containsKey(pathInContext))
					return (FilterChain) _chainCache[requestType]
							.get(pathInContext);
			}
		}
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#setContextAttribute
	...
	public synchronized void setContextAttribute(String name, Object value) {
		...
		if (_contextAttributeListeners != null) {
			ServletContextAttributeEvent event = new ServletContextAttributeEvent(
					getServletContext(), name, old != null ? old : value);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#
	 * removeContextAttribute(java.lang.String)
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Checks if is filter chains cached.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Cache filter chains. If true, filter chains are cached by the URI path
	 * within the context. Caching should not be used if the webapp encodes
	 * information in URLs.
	 * 
	 * @param filterChainsCached
	 ...
	 * @param o
	 *            the o
	 * @see org.browsermob.proxy.jetty.util.Container#addComponent(java.lang.Object)
	 */
	 ...
	 * @param o
	 *            the o
	 * @see org.browsermob.proxy.jetty.util.Container#removeComponent(java.lang.Object)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.jetty.servlet.ServletHandler#destroy()
	 */
	public void destroy() {
	...
	private static class FilterMapping {
		...
		/* ------------------------------------------------------------ */
		/**
		 * Instantiates a new filter mapping.
		 * 
		 * @param pathSpec
		 *            the path spec
		 * @param holder
		 ...
		 *            The path to check.
		 * @param type
		 *            The type of request: __REQUEST,__FORWARD,__INCLUDE or
		 *            __ERROR.
		 * @return True if this filter applies
		 */
		boolean appliesTo(String path, int type) {
			boolean b = ((_dispatches & type) != 0 || (_dispatches == 0 && type == Dispatcher.__REQUEST))
					&& (_pathSpec == null || PathMap.match(_pathSpec, path,
							true));
			return b;
		}
	}
	...
	private class Chain implements FilterChain {
		...
		public void doFilter(ServletRequest request, ServletResponse response)
			...
			if (_filter < LazyList.size(_filters)) {
				FilterHolder holder = (FilterHolder) LazyList.get(_filters,
						_filter++);
				if (log.isTraceEnabled())
	...
	private class CachedChain implements FilterChain {
		...
		/* ------------------------------------------------------------ */
		/**
		 * Instantiates a new cached chain.
		 * 
		 * @param filters
		 *            the filters
		 * @param servletHolder
	...
	public static void main(String[] arg) {
		...
		servletHolder.getServletContext().setAttribute("webApplication",
				"mWebApplication");
		servletHolder.getServletContext().setAttribute("applicationContext",
				"mApplicationContext");

		WebApplicationHandler mWebApplicationHandler = new WebApplicationHandler();

		ServletHolder servletHolder2 = mWebApplicationHandler.addServlet(
				"/mpath", "wicket.protocol.http.WicketServlet");
		servletHolder2.getServletContext().setAttribute("webApplication",
				"mWebApplication");
		servletHolder2.getServletContext().setAttribute("applicationContext",
				"mApplicationContext");
