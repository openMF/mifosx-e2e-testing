...
/* ---------------------------------------------------------------- */
/**
 * HTTP Http OutputStream. Acts as a BufferedOutputStream until setChunking() is
 * called. Once chunking is enabled, the raw stream is chunk encoded as per
 * RFC2616.
 * 
 ...
 * <LI>Notification of significant output events for filter triggering, header
 * flushing, etc.
 * </UL>
 * 
 ...
 * 
 * @version $Id: HttpOutputStream.java,v 1.28 2006/10/08 14:13:05 gregwilkins
 *          Exp $
 * @author Greg Wilkins
 */
public class HttpOutputStream extends OutputStream implements OutputObserver,
	...
	/* ------------------------------------------------------------ */
	/** The Constant __BUFFER_SIZE. */
	final static int __BUFFER_SIZE = 4096;

	/** The Constant __FIRST_RESERVE. */
	final static int __FIRST_RESERVE = 512;

	/** The Constant __filterArg. */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the buffered output stream.
	 * 
	 * @param bos
	 *            the new buffered output stream
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the backing output stream. A stream without filters or chunking is
	 * returned.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the output buffer size. Note that this is the minimal buffer size and
	 * that installed filters may perform their own buffering and are likely to
	 * change the size of the output. Also the pre and post reserve buffers may
	 * be allocated within the buffer for headers and chunking.
	 * 
	 * @param size
	 *            Minimum buffer size in bytes
	 * @throws IllegalStateException
	 *             If output has been written.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Reset Buffered output. If no data has been committed, the buffer output
	 * is discarded and the filters may be reinitialized.
	 * 
	 * @throws IllegalStateException
			...
			try {
				...
				if (_bufferedOut != null) {
					...
					if (_bufferedOut instanceof ChunkingOutputStream)
						((ChunkingOutputStream) _bufferedOut)
								.setChunking(false);
				}
			} catch (Exception e) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add an Output Observer. Output Observers get notified of significant
	 * events on the output stream. Observers are called in the reverse order
	 * they were added. They are removed when the stream is closed.
	 * 
	 * @param observer
	 *            The observer.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Null the output. All output written is discarded until the stream is
	 * reset. Used for HEAD requests.
	 * 
	 * @throws IOException
	 ...
	 *             Signals that an I/O exception has occurred.
	 * @throws IllegalStateException
	 *             The stream cannot be reset if chunking is enabled.
	 */
	...
	protected void checkOutput() {
		if (_out == _realOut) {
			if (_bufferedOut == null) {
				_bufferedOut = new ChunkingOutputStream(_realOut, _bufferSize,
						_headerReserve, false);
				_bufferedOut.setCommitObserver(this);
	...
	/* ------------------------------------------------------------ */
	/**
	 * Output Notification. Called by the internal Buffered Output and the event
	 * is passed on to this streams observers.
	 * 
	 * @param out
	 *            the out
	 * @param action
	...
	private void notify(int action) throws IOException {
		if (_observers != null) {
			for (int i = _observers.size(); i-- > 0;) {
				...
				((OutputObserver) _observers.get(i)).outputNotify(this, action,
						data);
			}
		}
	}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the checks if is o8859 writer.
	 * 
	 * @return the checks if is o8859 writer
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the uT f8 writer.
	 * 
	 * @return the uT f8 writer
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the aSCII writer.
	 * 
	 * @return the aSCII writer
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	 */
	...
	private class HttpWriter extends Writer {
		...
		/* -------------------------------------------------------- */
		/**
		 * Instantiates a new http writer.
		 * 
		 * @param encoding
		 *            the encoding
		 * @param bufferSize
