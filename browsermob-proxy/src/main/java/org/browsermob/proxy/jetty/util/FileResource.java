 ...
 * on if the platform does not have the "/" path separator, or it can be
 * controlled with the "org.mortbay.util.FileResource.checkAliases" system
 * parameter.
 * 
 * If alias checking is turned on, then aliased resources are treated as if they
 * do not exist, nor can they be created.
 * 
...
public class FileResource extends URLResource {
	...
	static {
		__checkAliases = "true".equalsIgnoreCase(System.getProperty(
				"org.browsermob.proxy.jetty.util.FileResource.checkAliases",
				"true"));
	 ...
	 * 
	 * @param checkAliases
	 *            True of resource aliases are to be checked for (eg case
	 *            insensitivity or 8.3 short names) and treated as not found.
	 */
	 ...
	 * getCheckAliases.
	 * 
	 * @return True of resource aliases are to be checked for (eg case
	 *         insensitivity or 8.3 short names) and treated as not found.
	 */
	...
	FileResource(URL url) throws IOException, URISyntaxException {
		...
		try {
			...
			try {
				...
				// encoding them.
				String urls = "file:"
						+ org.browsermob.proxy.jetty.util.URI.encodePath(url
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.util.URLResource#addPath(java.lang.String)
	 */
	public Resource addPath(String path) throws IOException,
		...
		if (expected != index
				&& ((expected - 1) != index || path.endsWith("/") || !r
						.isDirectory())) {
			r._alias = r._url;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.util.Resource#getAlias()
	 */
	public URL getAlias() {
