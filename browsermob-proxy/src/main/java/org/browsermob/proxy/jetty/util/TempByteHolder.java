...
/**
 * Temporary buffer for bytes to be used in situations where bytes need to be
 * buffered but total size of data is not known in advance and may potentially
 * be very large. Provides easy way to access small buffered data as byte[] or
 * String. Enables efficient memory-only handling of small data while
 * automatically switching to temporary file storage when data gets too big to
 * fit in memory buffer. It is highly efficient for both byte-per-byte and block
 * I/O. This class is not a FIFO - you can't mix reading and writing infinitely
 * as all data keep being buffered, not just unread data. Mixing reads and
 * writes may be inefficient in some situations but is fully supported. <br>
 * Overall usage strategy: You first write data to the buffer using OutputStream
 * returned by getOutputStream(), then examine data size using getLength() and
 * isLarge() and either call getBytes() to get byte[], getString() to get data
 * as String or getInputStream() to read data using stream. Instance of
 * TempByteHolder can be safely and efficiently reused by calling clear(). When
 * TempByteHolder is no longer needed you must call close() to ensure underlying
 * temporary file is closed and deleted. <br>
 * <br>
 * <i>NOTE:</i> For performance, this class is not synchronized. If you need
 * thread safety, use synchronized wrapper.<br>
 * This class can hold up to 2GB of data. <br>
 * <br>
 * <i>SECURITY NOTE:</i> As data may be written to disk, don't use this for
...
public class TempByteHolder {
	...
	TempByteHolder.InputStream _input_stream = null; // input_stream = new
														// TempByteHolder.InputStream();
	...
	/**
	 * Creates a new instance of TempByteHolder allocating memory buffer of
	 * given capacity. You should use reasonably large buffer for potentionally
	 * large data to improve effect of caching for file operations (about 512
	 * bytes).
	 * 
	 * @param in_memory_capacity
	 *            Size in bytes of memory buffer to allocate.
	 */
	...
	/**
	 * Creates a new instance of TempByteHolder using passed byte[] as memory
	 * buffer.
	 * 
	 * @param byte_array
	 *            byte array to be used as memory buffer.
	 */
	 ...
	 *            byte array to be used as memory buffer.
	 * @param offset
	 *            offset of prefilled data in buffer.
	 * @param prefilled_data_size
	...
	public TempByteHolder(byte[] byte_array, int offset, int prefilled_data_size) {
		...
		if ((offset < 0) || (offset > _window_size))
			throw new IllegalArgumentException("Bad prefilled data offset");
		if ((offset + prefilled_data_size > _window_size)
				|| (prefilled_data_size < 0))
	...
	/**
	 * Erases all unread buffered data and prepares for next use cycle. If
	 * temporary file was used, it is not closed/deleted yet as it may be needed
	 * again.
	 */
	public void clear() {
		_file_mode = false;
	...
	/**
	 * Truncates buffered data to specified size. Can not be used to extend
	 * data. Repositions OutputStream at the end of truncated data. If current
	 * read offset or mark is past the new end of data, it is moved at the new
	 * end.
	 * 
	 * @param offset
	 *            the offset
	...
	public void truncate(int offset) throws IOException {
		...
		if (_mark_pos > offset)
			_mark_pos = offset;
		_write_pos = offset;
		if (_file_high > offset)
			_file_high = offset;
		moveWindow(_write_pos);
	}

	/**
	 * Override directory to create temporary file in. Does not affect already
	 * open temp file.
	 * 
	 * @param dir
	 *            File object representing temporary directory. May be null
	 *            which means that system default (java.io.tmpdir system
	 ...
	 * 
	 * @return total number of bytes buffered. If you need number of bytes to be
	 *         read, use InputStream.available() .
	 */
	...
	/**
	 * Tells whether buffered data is small enough to fit in memory buffer so
	 * that it can be returned as byte[]. Data is considered large when it will
	 * not fit into backing memory buffer.
	 * 
	 * @return true when data is only accessible through InputStream interface;
	 *         false when data can be also retrieved directly as byte[] or
	 *         String.
	 * @see #getBytes()
	...
	/**
	 * Returns byte[] that holds all buffered data in its first getLength()
	 * bytes. If this instance was created using (byte[]) constructor, this is
	 * the same array that has been passed to the constructor. If buffered data
	 * don't fit into memory buffer, IllegalStateException is thrown.
	 * 
	 ...
	 *            Name of character encoding to use for converting bytes to
	 *            String.
	 ...
	 *            OutputStream to write to
	 * @param start_offset
	 *            Offset of data fragment to be written
	 * @param length
	 *            Length of data fragment to be written
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	 */
	public void writeTo(java.io.OutputStream os, int start_offset, int length)
			throws IOException {
	...
	private void createTempFile() throws IOException {
		_tempfilef = File.createTempFile(
				"org.browsermob.proxy.jetty.util.TempByteHolder-", ".tmp",
				_temp_directory).getCanonicalFile();
		_tempfilef.deleteOnExit();
		_tempfile = new RandomAccessFile(_tempfilef, "rw");
	}

	/**
	 * Write chunk of data at specified offset in temp file. Marks data as big.
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	private void readFromTempFile(int at_offset, byte[] data, int offset,
	...
	/**
	 * Move file window, synchronizing data with file. Works somewhat like
	 * memory-mapping a file. This one was nightmare to write :-)
	 * 
	 * @param start_offset
	 *            the start_offset
	 * @throws IOException
	...
	private void moveWindow(int start_offset) throws IOException {
		if (start_offset != _window_low) { // only when we have to move
			...
			if (dirty_len > 0) { // we need to be concerned at all about dirty
									// data.
				...
				if ((dirty_low < start_offset) || (dirty_high > end_offset)) {
					...
					writeToTempFile(dirty_low, _memory_buffer, dirty_low
							- _window_low, dirty_len);
				}
			}

			// reposition any data from old window that will be also in new
			// window:
			...
			if (stay_size > 0) {
				System.arraycopy(_memory_buffer, stay_low - _window_low,
						_memory_buffer, stay_low - start_offset, stay_size);
			}
			...
			if (stay_low > start_offset) {
				...
				if (toread_size > 0) {
					readFromTempFile(toread_low, _memory_buffer, toread_low
							- start_offset, toread_size);
				}
			}
	 ...
	 */
	private static boolean contained(int range1_low, int range1_high,
			int range2_low, int range2_high) {
		return ((range1_low >= range2_low) && (range1_high <= range2_high));
	}

	/**
	 * Internal implementation of java.io.OutputStream used to fill the byte
	 * buffer.
	 */
	class OutputStream extends java.io.OutputStream {
		...
		public void write(byte[] data, int off, int len) throws IOException {
			...
			boolean write_pos_in_window = (_write_pos >= _window_low)
					&& (_write_pos < _window_high);

			if (!write_pos_in_window) {
				// either current window is full of dirty data or it is
				// somewhere low
				moveWindow(_write_pos); // flush buffer if necessary, move
										// window at end
			}
			...
			if (end_of_data_in_window) {
				// if there is space in window for all data, just put it in
				// buffer.
				// 0 writes, window unchanged
				System.arraycopy(data, off, _memory_buffer, _write_pos
						- _window_low, len);
				...
				if (out_of_window < _window_size) {
					...
					// 1 write, window moved at window_high, filled with rest of
					// data

					// fill in rest of the current window with first part of
					// data
					int part1_len = _window_high - _write_pos;
					int part2_len = len - part1_len;

					System.arraycopy(data, off, _memory_buffer, _write_pos
							- _window_low, part1_len);
					_write_pos = _window_high;

					moveWindow(_write_pos); // flush data to file

					System.arraycopy(data, off + part1_len, _memory_buffer, 0,
							part2_len);
					_write_pos = new_write_pos;

				} else {
					// start of data in window, rest will not fit in window (and
					// leave some space):
					...
					if (part1_size == _window_size) {
						...
						if (part1_size > 0) {
							System.arraycopy(data, off, _memory_buffer,
									_write_pos - _window_low, part1_size);
							_write_pos += part1_size;
							moveWindow(_write_pos); // flush buffer
						}
						// flush window to file
						// write part 2 directly to file
						writeToTempFile(_write_pos, data, off + part1_size,
								part2_size);
						_write_pos = new_write_pos;
						moveWindow(_write_pos);
					}
	...
	class InputStream extends java.io.InputStream {
		...
		public int read(byte[] buff, int off, int len) throws IOException {
			...
			if (read_size > 0) {
				if (read_size >= _window_size) {
					...
					if (!contained(read_low, read_high, _window_low,
							_window_high)) {
						moveWindow(_read_pos);
					}
					System.arraycopy(_memory_buffer, _read_pos - _window_low,
							buff, off, read_size);
				}
				_read_pos += read_size;
			}
		...
		public long skip(long bytes) throws IOException {
			...
			int len = (int) bytes;
			if ((len + _read_pos) > _write_pos)
				len = _write_pos - _read_pos;
			_read_pos += len;
			moveWindow(_write_pos); // invalidate window without reading data by
									// moving it at the end
			return (long) len;
		}
