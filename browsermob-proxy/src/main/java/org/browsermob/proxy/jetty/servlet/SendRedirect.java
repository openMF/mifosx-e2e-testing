...
import org.browsermob.proxy.jetty.html.Heading;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (url != null && url.length() > 0) {
			...
			try {
				...
				TableForm tf = new TableForm(response.encodeURL(URI.addPaths(
						request.getContextPath(), request.getServletPath())
						+ "/action"));
				tf.method("GET");
				tf.addTextField("URL", "URL", 40, request.getContextPath()
						+ "/dump");
