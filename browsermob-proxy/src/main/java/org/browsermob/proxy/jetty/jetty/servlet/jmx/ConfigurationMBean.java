...
public class ConfigurationMBean extends ModelMBeanImpl {
	...
	/**
	 * Instantiates a new configuration m bean.
	 * 
	 * @throws MBeanException
	 *             the m bean exception
	 */
	public ConfigurationMBean() throws MBeanException {
	}

	/**
	 * defineManagedResource Grab the object which this mbean is proxying for,
	...
	/**
	 * getName This method is only defined to satisfy JMX: it is non-compliant
	 * to have an mbean with no methods on it, so this method has been added as
	 * a workaround.
	 * 
	 ...
	 * @see org.browsermob.proxy.jetty.util.jmx.ModelMBeanImpl#uniqueObjectName(javax.management.MBeanServer,
	 *      java.lang.String)
	 */
	public synchronized ObjectName uniqueObjectName(MBeanServer server,
		...
		try {
			oName = new ObjectName(on + ",config="
					+ _config.getClass().getName());
		} catch (Exception e) {
