...
/* ------------------------------------------------------------ */
/**
 * Dynamic Servlet Invoker. This servlet invokes anonymous servlets that have
 * not been defined in the web.xml or by other means. The first element of the
 * pathInfo of a request passed to the envoker is treated as a servlet name for
 * an existing servlet, or as a class name of a new servlet. This servlet is
 * normally mapped to /servlet/* This servlet support the following initParams:
 * 
 * <PRE>
...
public class Invoker extends HttpServlet {
	...
	public void init() {
		...
		while (e.hasMoreElements()) {
			...
			if ("nonContextServlets".equals(param)) {
				_nonContextServlets = value.length() > 0
						&& lvalue.startsWith("t");
			}
	...
	protected void service(HttpServletRequest request,
		...
		if (holder != null) {
			...
			_servletHandler.mapPathToServlet(
					URI.addPaths(servlet_path, servlet) + "/*",
					holder.getName());
		} else {
			...
			synchronized (_servletHandler) {
				...
				if (entry != null && entry != _invokerEntry) {
					...
					// Make a holder
					holder = new ServletHolder(_servletHandler, servlet,
							servlet);
					...
					if (!_nonContextServlets) {
						...
						if (_servletHandler.getClassLoader() != s.getClass()
								.getClassLoader()) {
							holder.stop();
					...
					_servletHandler.addServletHolder(holder);
					_servletHandler.mapPathToServlet(path + "/*",
							holder.getName());
					_servletHandler.mapPathToServlet(path + ".class/*",
							holder.getName());
				}
			}
		...
		if (holder != null)
			holder.handle(new Request(request, included, servlet, servlet_path,
					path_info), response);
		else
			response.sendError(404);
	...
	class Request extends HttpServletRequestWrapper {
		 ...
		 *            the path info
		 */
		Request(HttpServletRequest request, boolean included, String name,
		...
		public Object getAttribute(String name) {
			if (_included) {
				if (name.equals(Dispatcher.__INCLUDE_REQUEST_URI))
					return URI.addPaths(
							URI.addPaths(getContextPath(), _servletPath),
							_pathInfo);
