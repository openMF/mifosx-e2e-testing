...
/* ------------------------------------------------------------ */
/**
 * Handles coding of MIME "x-www-form-urlencoded". This class handles the
 * encoding and decoding for either the query string of a URL or the content of
 * a POST HTTP request.
 * 
 * <p>
 * <h4>Notes</h4>
...
public class UrlEncoded extends MultiMap {
	...
	/* ----------------------------------------------------------------- */
	/**
	 * Instantiates a new url encoded.
	 * 
	 * @param url
	 *            the url
	 */
	...
	public static void decodeTo(String content, MultiMap map, String charset) {
		...
		synchronized (map) {
			...
			for (int i = 0; i < content.length(); i++) {
				...
				switch (c) {
				case '&':
					value = encoded ? decodeString(content, mark + 1, i - mark
			...
			if (key != null) {
				value = encoded ? decodeString(content, mark + 1,
			...
			try {
				...
				while (ix < end) {
					...
					case '%':
						int i0 = (14 << 4) + 1;
						byte b0 = (byte) i0;
						data[ox++] = (byte) ((TypeUtil
								.convertHexDigit(data[ix++]) << 4) + TypeUtil
	 ...
	 * majority of calls will need no decoding and uses the 8859 encoding.
	 * 
	 * @param encoded
	 *            the encoded
	 * @return the string
	...
	public static String decodeString(String encoded) {
		return decodeString(encoded, 0, encoded.length(),
	 ...
	 * @return the string
	 */
	public static String decodeString(String encoded, int offset, int length,
			String charset) {
		if (charset == null)
			charset = StringUtil.__ISO_8859_1;
		byte[] bytes = null;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.util.MultiMap#clone()
	 */
	public Object clone() {
		return super.clone();
	}
