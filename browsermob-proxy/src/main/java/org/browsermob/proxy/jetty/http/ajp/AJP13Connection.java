...
public class AJP13Connection extends HttpConnection {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Instantiates a new aJ p13 connection.
	 * 
	 * @param listener
	 *            the listener
	 * @param in
	 ...
	 */
	public AJP13Connection(AJP13Listener listener, InputStream in,
			OutputStream out, Socket socket, int bufferSize) throws IOException {
		super(listener, null, new AJP13InputStream(in, out, bufferSize), out,
				socket);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.HttpConnection#handleNext()
	 */
	public boolean handleNext() {
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug("AJP13 type=" + type + " size="
						+ packet.unconsumedData());
			...
			case AJP13Packet.__FORWARD_REQUEST:
				...
				if (sc < 0)
					...
					request.setPath(URI.encodePath(path.substring(0, sc))
							+ path.substring(sc));
				...
				for (int i = 0; i < h; i++) {
					...
					if (!_keepAlive
							&& hdr.equalsIgnoreCase(HttpFields.__Connection)
				...
				while ((0xFF & attr) != 0xFF) {
					...
					case 9: // SSL session
						request.setAttribute(
								"javax.servlet.request.ssl_session", value);
						break;
					case 8: // SSL cipher
						request.setAttribute(
								"javax.servlet.request.cipher_suite", value);
						break;
					case 7: // SSL cert
					...
					case 6: // JVM Route
						request.setAttribute(
								"org.browsermob.proxy.jetty.http.ajp.JVMRoute",
								value);
						break;
					case 5: // Query String
				...
				response.setDateField(HttpFields.__Date,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.HttpConnection#firstWrite()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.HttpConnection#commit()
	 */
