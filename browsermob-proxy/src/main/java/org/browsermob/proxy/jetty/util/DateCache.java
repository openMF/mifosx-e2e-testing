...
public class DateCache {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the tz format string.
	 * 
	 * @param tz
	 *            the new tz format string
	 */
	...
	public synchronized String format(long inDate) {
		...
		if (seconds < _lastSeconds || _lastSeconds > 0
				&& seconds > _lastSeconds + __hitWindow) {
			// It's a cache miss
			_misses++;
