 ...
 * 
 * The log can contain multiple log syncs. The following system properties can
 * be used to control the configuration:
 * 
 * <pre>
 ...
 * 
 * @see org.browsermob.proxy.jetty.log.LogFactory
 */
public class LogImpl implements org.apache.commons.logging.Log {
	...
	/*-------------------------------------------------------------------*/
	/**
	 * Construct the shared instance of Log that decodes the options setup in
	...
	public LogImpl() {
		try {
			...
			System.err
					.println("Exception from getProperty!\n"
	...
	/*-------------------------------------------------------------------*/
	/**
	 * Default initialization is used the first time we have to log unless a
	 * sink has been added with add(). _needInit allows us to distinguish
	 * between initial state and disabled state.
	 */
	private synchronized void defaultInit() {
		if (!_initialized) {
			_initialized = true;
			String sinkClasses = System.getProperty("LOG_SINKS",
					"org.browsermob.proxy.jetty.log.OutputStreamLogSink");
			...
			while (sinkTokens.hasMoreTokens()) {
				...
				try {
					Class sinkClass = Loader.loadClass(this.getClass(),
							sinkClassName);
					if (org.browsermob.proxy.jetty.log.LogSink.class
							.isAssignableFrom(sinkClass)) {
						sink = (LogSink) sinkClass.newInstance();
						sink.start();
						add(sink);
					} else
						// Can't use Code.fail here, that's what we're setting
						// up
						System.err
								.println(sinkClass
										+ " is not a org.browsermob.proxy.jetty.log.LogSink");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	 ...
	 */
	public synchronized void message(String tag, Object msg, Frame frame,
			long time) {
		if (!_initialized)
			defaultInit();
	...
	/*-------------------------------------------------------------------*/
	/**
	 * Checks if is debug on for.
	 * 
	 * @param frame
	 *            the frame
	 * @return true, if is debug on for
	 */
		...
		if (_debugOn) {
			if (_debugPatterns == null)
				...
				for (int i = _debugPatterns.size(); --i >= 0;) {
					if (frame.getWhere()
							.indexOf((String) _debugPatterns.get(i)) >= 0)
						return true;
				}
			}
		}
