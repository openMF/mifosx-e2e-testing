...
/* ------------------------------------------------------------ */
/**
 * HTTP Server. Services HTTP requests by maintaining a mapping between a
 * collection of HttpListeners which generate requests and HttpContexts which
 * contain collections of HttpHandlers.
 * 
 * This class is configured by API calls. The org.mortbay.jetty.Server class
 * uses XML configuration files to configure instances of this class.
 * 
 * The HttpServer implements the BeanContext API so that membership events may
 * be generated for HttpListeners, HttpContexts and WebApplications.
 * 
 * @see HttpContext
 * @see HttpHandler
 * @see HttpConnection
 ...
 */
public class HttpServer extends Container implements LifeCycle, EventProvider,
		Serializable {
	...
	/** The __ro servers. */
	private static Collection __roServers = Collections
			.unmodifiableCollection(__servers.keySet());

	/** The __no virtual host. */
	private static String[] __noVirtualHost = new String[1];

	/* ------------------------------------------------------------ */
	/**
	 * Get HttpServer Collection. Get a collection of all known HttpServers.
	 * Servers can be removed from this list with the setAnonymous call.
	 * 
	...
	/** The _requests per gc. */
	private int _requestsPerGC;

	/** The _resolve remote host. */
	 ...
	 * 
	 * @param anonymous
	 *            If true, the server is not included in the static server lists
	 *            and stopAll methods.
	 */
	 ...
	 *             If the listener is not for this server.
	 */
	...
	public synchronized HttpContext[] getContexts() {
		...
		return (HttpContext[]) contexts
				.toArray(new HttpContext[contexts.size()]);
	}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add a context. As contexts cannot be publicly created, this may be used
	 * to alias an existing context.
	 * 
	 * @param virtualHost
	 *            The virtual host or null for all hosts.
	 * @param context
	 *            the context
	 * @return the http context
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Create and add a new context. Note that multiple contexts can be created
	 * for the same virtualHost and contextPath. Requests are offered to
	 * multiple contexts in the order they where added to the HttpServer.
	 * 
	 * @param contextPath
	 *            the context path
	 ...
	 * 
	 * @param virtualHost
	 *            Virtual hostname or null for all hosts.
	 ...
	 * @return HttpContext. If multiple contexts exist for the same virtualHost
	 *         and pathSpec, the most recently added context is returned. If no
	 *         context exists, a new context is created by a call to
	 *         newHttpContext.
	 */
	public HttpContext getContext(String virtualHost, String contextPathSpec) {
		HttpContext hc = null;
	 ...
	 * @param contextPathSpec
	 *            Path specification relative to the context path.
	 * @return The HttpContext If multiple contexts exist for the same pathSpec,
	 *         the most recently added context is returned. If no context
	 *         exists, a new context is created by a call to newHttpContext.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Create a new HttpContext. Specialized HttpServer classes may override
	 * this method to return subclasses of HttpContext.
	 * 
	...
	synchronized void addMapping(String virtualHost, HttpContext context) {
		...
		String contextPathSpec = HttpContext.canonicalContextPathSpec(context
				.getContextPath());
		...
		if (log.isDebugEnabled())
			log.debug("Added " + context + " for host "
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the requests per GC. If this is set greater than zero, then the
	 * System garbage collector will be invoked after approximately this number
	 * of requests. For predictable response, it is often best to have frequent
	 * small runs of the GC rather than infrequent large runs. The request count
	 * is only approximate as it is not synchronized and multi CPU machines may
	 * miss counting some requests.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the requests per GC. If this is set greater than zero, then the
	 * System garbage collector will be invoked after approximately this number
	 * of requests. For predictable response, it is often best to have frequent
	 * small runs of the GC rather than infrequent large runs. The request count
	 * is only approximate as it is not synchronized and multi CPU machines may
	...
	/* ------------------------------------------------------------ */
	/**
	 * Start all handlers then listeners. If a subcomponent fails to start, it's
	 * exception is added to a org.mortbay.util.MultiException and the start
	 * method continues.
	 * 
	 * @throws Exception
	 *             the exception
	 ...
	 *             If interrupted, stop may not have been called on everything.
	 */
	 ...
	 * 
	 * @param graceful
	 *            If true and statistics are on for a context, then this method
	 *            will wait for requestsActive to go to zero before stopping
	 *            that context.
	 * @throws InterruptedException
	...
	/* ------------------------------------------------------------ */
	/**
	 * Define a virtual host alias. All requests to the alias are handled the
	 * same as request for the virtualHost.
	 * 
	 * @param virtualHost
	 *            Host name or IP
	 * @param alias
	...
	/* ------------------------------------------------------------ */
	/**
	 * Service a request. Handle the request by passing it to the HttpHandler
	 * contained in the mapped HttpContexts. The requests host and path are used
	 * to select a list of HttpContexts. Each HttpHandler in these context is
	 * offered the request in turn, until the request is handled.
	 * 
	...
	public HttpContext service(HttpRequest request, HttpResponse response)
			...
			if (contextMap != null) {
				...
				if (contextLists != null) {
					if (log.isTraceEnabled())
						log.trace("Contexts at " + request.getPath() + ": "
								+ contextLists);
					...
					for (int i = 0; i < contextLists.size(); i++) {
						...
						for (int j = 0; j < contextList.size(); j++) {
							HttpContext context = (HttpContext) contextList
									.get(j);
		...
		synchronized (this) {
			if (_notFoundContext == null) {
				...
				try {
					_notFoundContext
							.addHandler((NotFoundHandler) Class
									.forName(
											"org.browsermob.proxy.jetty.http.handler.RootNotFoundHandler")
									.newInstance());
				} catch (Exception e) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Find handler. Find a handler for a URI. This method is provided for the
	 * servlet context getContext method to search for another context by URI. A
	 * list of hosts may be passed to qualify the search.
	 * 
	 * @param handlerClass
	 *            the handler class
	 * @param uri
	 *            URI that must be satisfied by the servlet handler
	 * @param vhosts
	 *            null or a list of virtual hosts names to search
	 * @return HttpHandler
	 */
	public HttpHandler findHandler(Class handlerClass, String uri,
			String[] vhosts) {
	 ...
	 * @return The named realm. If the name is null and only a single realm is
	 *         known, that is returned.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the resolve remote host.
	 * 
	 * @param resolveRemoteHost
	...
	/** The _connections. */
	private transient int _connections; // total number of connections made to
										// server

	/** The _connections open. */
	private transient int _connectionsOpen; // number of connections currently
											// open

	/** The _connections open min. */
	private transient int _connectionsOpenMin; // min number of connections open
												// simultaneously

	/** The _connections open max. */
	private transient int _connectionsOpenMax; // max number of connections open
												// simultaneously

	/** The _connections duration min. */
	private transient long _connectionsDurationMin; // min duration of a
													// connection

	/** The _connections duration max. */
	private transient long _connectionsDurationMax; // max duration of a
													// connection

	/** The _connections duration total. */
	private transient long _connectionsDurationTotal; // total duration of all
														// coneection
	...
	private transient int _requestsActive; // number of requests currently being
											// handled

	/** The _requests active min. */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections duration min.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections duration total.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections requests min.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests duration min.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests duration total.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections duration ave.
	 * 
	 * @return Average duration in milliseconds of open connections since
	 *         statsReset() called. Undefined if setStatsOn(false).
	 */
	public long getConnectionsDurationAve() {
		return _connections == 0 ? 0
				: (_connectionsDurationTotal / _connections);
	}

	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections duration max.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections requests ave.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the connections requests max.
	 * 
	 ...
	 * Gets the errors.
	 * 
	 * @return Number of errors generated while handling requests. since
	 *         statsReset() called. Undefined if setStatsOn(false).
	 */
	 ...
	 * Gets the requests.
	 * 
	 * @return Number of requests since statsReset() called. Undefined if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests duration ave.
	 * 
	 * @return Average duration of request handling in milliseconds since
	 *         statsReset() called. Undefined if setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests duration max.
	 * 
	 * @return Get maximum duration in milliseconds of request handling since
	 *         statsReset() called. Undefined if setStatsOn(false).
	 */
	...
	void statsEndRequest(long duration, boolean ok) {
		synchronized (_statsLock) {
			...
			if (ok) {
				...
				if (_requestsDurationMin == 0
						|| duration < _requestsDurationMin)
	...
	void statsCloseConnection(long duration, int requests) {
		synchronized (_statsLock) {
			...
			if (_connectionsOpen < _connectionsOpenMin)
				_connectionsOpenMin = _connectionsOpen;
			if (_connectionsDurationMin == 0
					|| duration < _connectionsDurationMin)
				_connectionsDurationMin = duration;
			if (duration > _connectionsDurationMax)
				_connectionsDurationMax = duration;
			if (_connectionsRequestsMin == 0
					|| requests < _connectionsRequestsMin)
	...
	/* ------------------------------------------------------------ */
	/**
	 * Save the HttpServer The server is saved by serialization to the given
	 * filename or URL.
	 * 
	 * @param saveat
