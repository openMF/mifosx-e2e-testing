...
import org.browsermob.proxy.jetty.html.*;
import org.browsermob.proxy.jetty.log.LogFactory;
import org.browsermob.proxy.jetty.log.LogImpl;
import org.browsermob.proxy.jetty.log.LogSink;
import org.browsermob.proxy.jetty.log.OutputStreamLogSink;
...
public class Debug extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		tf.table()
				.newRow()
				.addCell(
						new Block(Block.Bold).add(new Font(3, true)
								.add(getServletInfo()))).cell()
				.attribute("COLSPAN", "2");
		...
		for (int s = 0; sinks != null && s < sinks.length; s++) {
			...
			tf.table().newRow().addCell(Break.rule).cell()
					.attribute("COLSPAN", "2");
			tf.table()
					.newRow()
					.addCell(
							"<B><font size=\"+1\">Log Sink " + s
									+ ":</font></B").right();
			...
			if (sinks[s] instanceof OutputStreamLogSink) {
				...
				tf.addCheckbox("SS" + s, "Suppress Stacks",
						sink.isSuppressStack());
				tf.addCheckbox("SL" + s, "Single Line", sink.isLogOneLine());
				tf.addTextField("LF" + s, "Log File Name", 40,
						sink.getFilename());
			}
		}
		...
		tf.addTextField("LSC", "Add LogSink Class", 40,
		...
		if ("Set Options".equals(action)) {
			...
			for (int s = 0; sinks != null && s < sinks.length; s++) {
				...
				if (sinks[s] instanceof OutputStreamLogSink) {
					...
					sink.setLogLabels("on".equals(request
							.getParameter("LL" + s)));
					sink.setLogStackSize("on".equals(request.getParameter("Ls"
							+ s)));
					sink.setLogStackTrace("on".equals(request.getParameter("LS"
							+ s)));
					sink.setSuppressStack("on".equals(request.getParameter("SS"
							+ s)));
					sink.setLogOneLine("on".equals(request.getParameter("SL"
							+ s)));
