 ...
 * TagLibConfiguration.
 * 
 * The class searches for TLD descriptors found in web.xml, in WEB-INF/*.tld
 * files of the web app or *.tld files withing jars found in WEB-INF/lib of the
 * webapp. Any listeners defined in these tld's are added to the context.
 * 
 * &lt;bile&gt;This is total rubbish special case for JSPs! If there was a
 * general use-case for web app frameworks to register listeners directly, then
 * a generic mechanism could have been added to the servlet spec. Instead some
 * special purpose JSP support is required that breaks all sorts of
 * encapsualtion rules as the servlet container must go searching for and then
 * parsing the descriptors for one particular framework. It only appears to be
 * used by JSF, which is being developed by the same developer who implemented
 * this feature in the first place! &lt;/bile&gt;
 * 
 * @author gregw
 * 
 */
public class TagLibConfiguration implements Configuration {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Instantiates a new tag lib configuration.
	 */
