 ...
 * <P>
 * ISO_8859_1 encoding is used by default for % encoded characters. This may be
...
public class URI implements Cloneable {
	...
	/** The Constant __CHARSET. */
	public static final String __CHARSET = System.getProperty(
			"org.browsermob.proxy.jetty.util.URI.charset", StringUtil.__UTF_8);

	/** The Constant __CHARSET_IS_DEFAULT. */
	public static final boolean __CHARSET_IS_DEFAULT = __CHARSET
			.equals(StringUtil.__UTF_8);
	...
	/* ------------------------------------------------------------ */
	/**
	 * Construct from a String. The string must contain a URI path, but
	 * optionaly may contain a scheme, host, port and query string.
	 * 
	 * @param uri
	 ...
	 *            name of value
	 * @param value
	 *            The value, which may be a multi valued list or String array.
	 * @return the object
	 */
	...
	public void remove(String name) {
		if (_parameters != null) {
			_dirty = _parameters.remove(name) != null;
		}
	}
	 ...
	 * @return The StringBuffer or null if no substitutions required.
	 */
	public static StringBuffer encodeString(StringBuffer buf, String path,
			String encode) {
		if (buf == null) {
			loop: for (int i = 0; i < path.length(); i++) {
				char c = path.charAt(i);
	 ...
	 * factored out. Null is returned if the path tries to .. above it's root.
	 * 
	 * @param path
	 *            the path
	 * @return path or null.
