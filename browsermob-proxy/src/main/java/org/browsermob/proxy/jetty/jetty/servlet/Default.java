 ...
 * following initParameters are supported:
 * 
 * <PRE>
 * 
...
public class Default extends HttpServlet {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the inits the boolean.
	 * 
	 * @param name
	 *            the name
	 * @return the inits the boolean
	 */
	private boolean getInitBoolean(String name) {
		String value = getInitParameter(name);
		return value != null
				&& value.length() > 0
				&& (value.startsWith("t") || value.startsWith("T")
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the inits the int.
	 * 
	 * @param name
	 *            the name
	 * @return the inits the int
	 */
	...
	protected Resource getResource(String pathInContext) throws IOException {
		Resource r = (_resourceBase == null) ? _httpContext
				.getResource(pathInContext) : _resourceBase
	...
	protected void service(HttpServletRequest request,
		...
		String servletPath = (String) request
				.getAttribute(Dispatcher.__INCLUDE_SERVLET_PATH);
		...
		if (servletPath == null) {
			...
			pathInfo = (String) request
					.getAttribute(Dispatcher.__INCLUDE_PATH_INFO);
		...
		try {
			...
			if (method.equals(HttpRequest.__GET)
				...
				handleGet(request, response, pathInContext, resource,
						endsWithSlash);
			...
			else if (_putAllowed && _delAllowed
	...
	public void handleGet(HttpServletRequest request,
			HttpServletResponse response, String pathInContext,
			Resource resource, boolean endsWithSlash) throws ServletException,
			IOException {
		if (resource == null || !resource.exists())
			response.sendError(HttpResponse.__404_Not_Found);
		else {
			...
			if (resource.isDirectory()) {
				...
				if (welcome != null) {
					...
					if (_redirectWelcomeFiles) {
						...
						response.setContentLength(0);
						response.sendRedirect(URI.addPaths(
								_httpContext.getContextPath(), ipath));
					} else {
						// Forward to the index
						RequestDispatcher dispatcher = _servletHandler
								.getRequestDispatcher(ipath);
	...
	protected boolean passConditionalHeaders(HttpServletRequest request,
		...
		if (!request.getMethod().equals(HttpRequest.__HEAD)
			...
			// parsing the date.
			ResourceCache.ResourceMetaData metaData = _httpContext
	...
	protected void sendData(HttpServletRequest request,
		...
		boolean include = request
				.getAttribute(Dispatcher.__INCLUDE_REQUEST_URI) != null;
		...
		Enumeration reqRanges = include ? null : request
				.getHeaders(HttpFields.__Range);
			...
			if (!include) {
				...
				if (_minGzipLength > 0) {
					String accept = request
							.getHeader(HttpFields.__AcceptEncoding);
					if (accept != null && resLength > _minGzipLength
							&& !pathInContext.endsWith(".gz")) {
						Resource gz = getResource(pathInContext + ".gz");
						if (gz.exists()
								&& accept.indexOf("gzip") >= 0
								&& request
										.getAttribute(Dispatcher.__INCLUDE_REQUEST_URI) == null) {
							response.setHeader(HttpFields.__ContentEncoding,
									"gzip");
							data = gz;
		...
		MultiPartResponse multi = new MultiPartResponse(
	...
	protected void writeHeaders(HttpServletResponse response,
			Resource resource, long count) throws IOException {
		ResourceCache.ResourceMetaData metaData = _httpContext
				.getResourceMetaData(resource);
		...
		if (count != -1) {
			if (count == resource.length()
					&& response instanceof ServletHttpResponse)
		...
		response.setHeader(HttpFields.__LastModified,
