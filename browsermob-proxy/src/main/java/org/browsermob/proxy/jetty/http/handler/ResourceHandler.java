...
/* ------------------------------------------------------------ */
/**
 * Handler to serve files and resources. Serves files from a given resource URL
 * base and implements the GET, HEAD, DELETE, OPTIONS, PUT, MOVE methods and the
 * IfModifiedSince and IfUnmodifiedSince header fields. A simple memory cache is
 * also provided to reduce file I/O. HTTP/1.1 ranges are supported.
 * 
...
public class ResourceHandler extends AbstractHttpHandler {
	...
	/** The _min gzip length. */
	private int _minGzipLength = -1;

	/** The _method map. */
	private StringMap _methodMap = new StringMap();
	{
		setAllowedMethods(new String[] { HttpRequest.__GET, HttpRequest.__POST,
				HttpRequest.__HEAD, HttpRequest.__OPTIONS, HttpRequest.__TRACE });
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.handler.AbstractHttpHandler#stop()
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Checks if is dir allowed.
	 * 
	 * @return true, if is dir allowed
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the redirect welcome.
	 * 
	 * @return True if welcome files are redirected to. False if forward is
	 *         used.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the redirect welcome.
	 * 
	 * @param redirectWelcome
	 *            True if welcome files are redirected to. False if forward is
	 *            used.
	 */
	 ...
	 * 
	 * @param minGzipLength
	 *            If set to a positive integer, then static content larger than
	 *            this will be served as gzip content encoded if a matching
	 ...
	 * implementation calls HttpContext.getResource but derived handers may
	...
	public void handle(String pathInContext, String pathParams,
		...
		try {
			...
			if (method.equals(HttpRequest.__GET)
				...
				handleGet(request, response, pathInContext, pathParams,
						resource);
	...
	public void handleGet(HttpRequest request, HttpResponse response,
			String pathInContext, String pathParams, Resource resource)
			throws IOException {
		if (log.isDebugEnabled())
			...
			if (resource.isDirectory()) {
				...
				if (welcome != null) {
					...
					if (_redirectWelcomeFiles) {
						...
						ipath = URI.addPaths(getHttpContext().getContextPath(),
								ipath);
						...
						getHttpContext().handle(ipath, pathParams, request,
								response);
					}
					return;
				}
	...
	private boolean passConditionalHeaders(HttpRequest request,
			HttpResponse response, Resource resource) throws IOException {
		if (!request.getMethod().equals(HttpRequest.__HEAD)) {
			...
			// parsing the date.
			ResourceCache.ResourceMetaData metaData = (ResourceCache.ResourceMetaData) resource
					.getAssociate();
			if (metaData != null) {
	...
	void handlePut(HttpRequest request, HttpResponse response,
			String pathInContext, Resource resource) throws IOException {
		if (log.isDebugEnabled())
		...
		if (pathInContext.endsWith("/")) {
			...
			try {
				...
				out.close();
				request.setHandled(true);
				response.setStatus(exists ? HttpResponse.__200_OK
	...
	void handleMove(HttpRequest request, HttpResponse response,
		...
		try {
			...
			Resource newFile = getHttpContext().getBaseResource().addPath(
					newInfo);
			...
			log.warn(LogSupport.EXCEPTION, ex);
			setAllowHeader(response);
			response.sendError(HttpResponse.__405_Method_Not_Allowed, "Error:"
					+ ex);
			return;
		}
	}
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public void writeHeaders(HttpResponse response, Resource resource,
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public void sendData(HttpRequest request, HttpResponse response,
			String pathInContext, Resource resource, boolean writeHeaders)
			throws IOException {
		long resLength = resource.length();

		// see if there are any range headers
		Enumeration reqRanges = request.getDotVersion() > 0 ? request
				.getFieldValues(HttpFields.__Range) : null;
		...
		if (!writeHeaders || reqRanges == null || !reqRanges.hasMoreElements()) {
			...
			if (_minGzipLength > 0) {
				...
				if (accept != null && resLength > _minGzipLength
					...
					Resource gz = getHttpContext().getResource(
							pathInContext + ".gz");
		...
		List ranges = InclusiveByteRange
				.satisfiableRanges(reqRanges, resLength);
		if (log.isDebugEnabled())
		...
		if (ranges == null || ranges.size() == 0) {
			...
			response.setReason((String) HttpResponse.__statusMsg.get(TypeUtil
					.newInteger(HttpResponse.__416_Requested_Range_Not_Satisfiable)));
		...
		if (ranges.size() == 1) {
			...
			response.setReason((String) HttpResponse.__statusMsg.get(TypeUtil
					.newInteger(HttpResponse.__206_Partial_Content)));
			response.setField(HttpFields.__ContentRange,
					singleSatisfiableRange.toHeaderRangeString(resLength));
			OutputStream out = response.getOutputStream();
			resource.writeTo(out, singleSatisfiableRange.getFirst(resLength),
					singleLength);
			request.setHandled(true);
			return;
		}
		...
		InputStream in = (resource instanceof CachedResource) ? null : resource
				.getInputStream();
		OutputStream out = response.getOutputStream();
		long pos = 0;
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	void sendDirectory(HttpRequest request, HttpResponse response,
			Resource resource, boolean parent) throws IOException {
		if (!_dirAllowed) {
			response.sendError(HttpResponse.__403_Forbidden);
			return;
		}
