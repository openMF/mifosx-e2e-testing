...
import org.browsermob.proxy.jetty.jetty.servlet.WebApplicationContext;
 ...
 * 
 * This specialization of org.mortbay.http.HttpServer adds knowledge about
 * servlets and their specialized contexts. It also included support for
 * initialization from xml configuration files that follow the XmlConfiguration
 * dtd.
 * 
 * HttpContexts created by Server are of the type
 * org.mortbay.jetty.servlet.ServletHttpContext unless otherwise specified.
 * 
 * This class also provides a main() method which starts a server for each
 * config file passed on the command line. If the system property
 * JETTY_NO_SHUTDOWN_HOOK is not set to true, then a shutdown hook is thread is
 * registered to stop these servers.
 * 
 * @see org.browsermob.proxy.jetty.xml.XmlConfiguration
...
public class Server extends HttpServer {
	...
	/** The _web app configuration class names. */
	private String[] _webAppConfigurationClassNames = new String[] {
			"org.browsermob.proxy.jetty.jetty.servlet.XMLConfiguration",
			"org.browsermob.proxy.jetty.jetty.servlet.JettyWebConfiguration" };
	 ...
	 * 
	 * @param configuration
	 *            The filename or URL of the XML configuration file.
	 * @throws IOException
	...
	public Server(URL configuration) throws IOException {
		...
		try {
			...
			throw new IOException("Jetty configuration problem: "
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the stop at shutdown.
	 * 
	 * @return the stop at shutdown
	 */
	...
	public void configure(String configuration) throws IOException {
		...
		if (_configuration != null)
			throw new IllegalStateException("Already configured with "
					+ _configuration);
		try {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Create a new ServletHttpContext. Ths method is called by HttpServer to
	 * creat new contexts. Thus calls to addContext or getContext that result in
	 * a new Context being created will return an
	...
	/* ------------------------------------------------------------ */
	/**
	 * Create a new WebApplicationContext. Ths method is called by Server to
	 * creat new contexts for web applications. Thus calls to addWebApplication
	 * that result in a new Context being created will return an correct class
	 * instance. Derived class can override this method to create instance of
	 * its own class derived from WebApplicationContext in case it needs more
	 * functionality.
	 * 
	 * @param webApp
	...
	public WebApplicationContext addWebApplication(String virtualHost,
			String contextPathSpec, String webApp) throws IOException {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add Web Applications. Add auto webapplications to the server. The name of
	 * the webapp directory or war is used as the context name. If a webapp is
	 * called "root" it is added at "/".
	 * 
	 * @param webapps
	...
	/**
	 * Add Web Applications. Add auto webapplications to the server. The name of
	 * the webapp directory or war is used as the context name. If the webapp
	 * matches the rootWebApp it is added as the "/" context.
	 * 
	 * @param host
	 *            Virtual host name or null
	 * @param webapps
	...
	public WebApplicationContext[] addWebApplications(String host,
			String webapps) throws IOException {
	 ...
	 */
	public WebApplicationContext[] addWebApplications(String host,
			String webapps, boolean extract) throws IOException {
	 ...
	 *            Directory file name or URL to look for auto webapplication.
	 * @param defaults
	 *            The defaults xml filename or URL which is loaded before any in
	 *            the web app. Must respect the web.dtd. If null the default
	 *            defaults file is used. If the empty string, then no defaults
	 *            file is used.
	 * @param extract
	 *            If true, extract war files
	 * @return the web application context[]
	 ...
	 */
	public WebApplicationContext[] addWebApplications(String host,
			String webapps, String defaults, boolean extract)
			throws IOException {
	 ...
	 * @param extract
	 *            If true, extract war files
	 * @param java2CompliantClassLoader
	...
	public WebApplicationContext[] addWebApplications(String host,
		...
		for (int f = 0; files != null && f < files.length; f++) {
			...
			if (_rootWebApp != null
					&& (context.equals(_rootWebApp) || context
							.equals(_rootWebApp + "/")))
				context = "/";
			else
				context = "/" + context;
		...
		return (WebApplicationContext[]) wacs
				.toArray(new WebApplicationContext[wacs.size()]);
	}

	/* ------------------------------------------------------------ */
	/**
	 * setWebApplicationConfigurationClasses Set up the list of classnames of
	 * WebApplicationContext.Configuration implementations that will be applied
	 * to configure every webapp. The list can be overridden by individual
	 * WebApplicationContexts.
	 * 
	 * @param configurationClassNames
	 *            the new web application configuration class names
	 */
	public void setWebApplicationConfigurationClassNames(
		...
		if (configurationClassNames != null) {
			...
			System.arraycopy(configurationClassNames, 0,
					_webAppConfigurationClassNames, 0,
					configurationClassNames.length);
		}
	}

	/**
	 * Gets the web application configuration class names.
	 * 
	 * @return the web application configuration class names
	 */
	...
	private static class ShutdownHookThread extends Thread {
		...
		private void createShutdownHook() {
			if (!Boolean.getBoolean("JETTY_NO_SHUTDOWN_HOOK") && !hooked) {
				try {
					Method shutdownHook = java.lang.Runtime.class.getMethod(
							"addShutdownHook",
