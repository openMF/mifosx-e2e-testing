 ...
 * <p>
 * This class decodes the multipart/form-data stream sent by a HTML form that
 * uses a file input item.
 * 
 * <p>
 * <h4>Usage</h4>
 * Each part of the form data is named from the HTML form and is available
 * either via getString(name) or getInputStream(name). Furthermore the MIME
 * parameters and filename can be requested for each part.
 * 
 * <pre>
 * </pre>
 * 
 * @version $Id: MultiPartRequest.java,v 1.16 2005/12/02 20:13:52 gregwilkins
 *          Exp $
 * @author Greg Wilkins
 * @author Jim Crossley
 */
public class MultiPartRequest {
	 ...
	 * 
	 * @param request
	 *            The request containing a multipart/form-data request
	 * @throws IOException
	 *             IOException
	 */
	...
	public String getString(String name) {
		...
		if (_encoding != null) {
			try {
				...
				if (log.isDebugEnabled())
					log.debug("Invalid character set: " + uee);
				return null;
			}
		} else
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the multiple params.
	 * 
	 * @param name
	 *            the name
	 * @return the multiple params
	 */
		...
		while (!_lastPart) {
			...
			while ((line = _in.readLine()) != null) {
				...
				if (c > 0) {
					...
					part._headers.put(key, (ev != null) ? (ev + ';' + value)
							: value);
					if (log.isDebugEnabled())
						log.debug(key + ": " + value);
	 ...
	 * Value.
	 * 
	 * @param nameEqualsValue
	 *            the name equals value
	 * @return the string
	 */
