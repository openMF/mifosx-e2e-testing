...
/* ------------------------------------------------------------ */
/**
 * FORM Authentication Authenticator. The HTTP Session is used to store the
 * authentication status of the user, which can be distributed. If the realm
 * implements SSORealm, SSO is supported.
 * 
 * @version $Id: FormAuthenticator.java,v 1.32 2005/08/13 00:01:27 gregwilkins
 *          Exp $
...
public class FormAuthenticator implements Authenticator {
	...
	/* ------------------------------------------------------------ */
	/** The Constant __J_URI. */
	public final static String __J_URI = "org.browsermob.proxy.jetty.jetty.URI";

	/** The Constant __J_AUTHENTICATED. */
	public final static String __J_AUTHENTICATED = "org.browsermob.proxy.jetty.jetty.Auth";

	/** The Constant __J_SECURITY_CHECK. */
	public final static String __J_SECURITY_CHECK = "j_security_check";

	/** The Constant __J_USERNAME. */
	public final static String __J_USERNAME = "j_username";

	/** The Constant __J_PASSWORD. */
	public final static String __J_PASSWORD = "j_password";

	/** The _form error page. */
	private String _formErrorPage;

	/** The _form error path. */
	private String _formErrorPath;

	/** The _form login page. */
	private String _formLoginPage;

	/** The _form login path. */
	...
	public void setLoginPage(String path) {
		...
		if (_formLoginPath.indexOf('?') > 0)
			_formLoginPath = _formLoginPath.substring(0,
					_formLoginPath.indexOf('?'));
	}
	...
	public void setErrorPage(String path) {
		if (path == null || path.trim().length() == 0) {
			...
			if (_formErrorPath != null && _formErrorPath.indexOf('?') > 0)
				_formErrorPath = _formErrorPath.substring(0,
						_formErrorPath.indexOf('?'));
		}
	}
	...
	public Principal authenticate(UserRealm realm, String pathInContext,
			...
			throws IOException {
		HttpServletRequest request = (ServletHttpRequest) httpRequest
				.getWrapper();
		...
		if (uri.substring(uri.lastIndexOf("/") + 1).startsWith(
				__J_SECURITY_CHECK)) {
			// Check the session object for login info.
			FormCredential form_cred = new FormCredential();
			form_cred.authenticate(realm, request.getParameter(__J_USERNAME),
					request.getParameter(__J_PASSWORD), httpRequest);
			...
			if (form_cred._userPrincipal != null) {
				...
				if (log.isDebugEnabled())
					log.debug("Form authentication OK for "
							+ form_cred._jUserName);
				...
				if (realm instanceof SSORealm) {
					((SSORealm) realm).setSingleSignOn(httpRequest,
							httpResponse, form_cred._userPrincipal,
				...
				if (log.isDebugEnabled())
					log.debug("Form authentication FAILED for "
							+ form_cred._jUserName);
				if (_formErrorPage != null) {
					response.setContentLength(0);
					response.sendRedirect(response.encodeRedirectURL(URI
							.addPaths(request.getContextPath(), _formErrorPage)));
				} else {
		...
		FormCredential form_cred = (FormCredential) session
				.getAttribute(__J_AUTHENTICATED);
		...
		if (form_cred != null) {
			...
			Credential cred = ((SSORealm) realm).getSingleSignOn(httpRequest,
					httpResponse);
		...
		if (response != null) {
			...
			response.setContentLength(0);
			response.sendRedirect(response.encodeRedirectURL(URI.addPaths(
					request.getContextPath(), _formLoginPage)));
		}
	...
	/**
	 * Checks if is login or error page.
	 * 
	 * @param pathInContext
	 *            the path in context
	 * @return true, if is login or error page
	 */
	public boolean isLoginOrErrorPage(String pathInContext) {
		return pathInContext != null
				&& (pathInContext.equals(_formErrorPath) || pathInContext
						.equals(_formLoginPath));
	}
	...
	private static class FormCredential implements Serializable,
		 ...
		 *            the request
		 */
		void authenticate(UserRealm realm, String user, String password,
				HttpRequest request) {
			_jUserName = user;
			_jPassword = password;
		...
		void authenticate(UserRealm realm, HttpRequest request) {
			_userPrincipal = realm
					.authenticate(_jUserName, _jPassword, request);
			if (_userPrincipal != null)
				_realm = realm;
		}
		...
		public boolean equals(Object o) {
			...
			return _jUserName.equals(fc._jUserName)
