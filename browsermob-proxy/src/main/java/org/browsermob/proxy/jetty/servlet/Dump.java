...
public class Dump extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (info != null && info.indexOf("Locale/") >= 0) {
			try {
				String locale_name = info
						.substring(info.indexOf("Locale/") + 7);
		...
		try {
			...
			table.newRow();
			table.newHeading().cell().nest(new Font(2, true))
					.add("<BR>Other HTTP Headers").attribute("COLSPAN", "2")
					.left();
			...
			table.newRow();
			table.newHeading().cell().nest(new Font(2, true))
					.add("<BR>Request Parameters").attribute("COLSPAN", "2")
					.left();
			...
			table.newRow();
			table.newHeading().cell().nest(new Font(2, true))
					.add("<BR>Cookies").attribute("COLSPAN", "2").left();
			Cookie[] cookies = request.getCookies();
			for (int i = 0; cookies != null && i < cookies.length; i++) {
				Cookie cookie = cookies[i];

				table.newRow();
				table.addHeading(cookie.getName() + ":&nbsp;").cell()
						.attribute("VALIGN", "TOP").right();
			...
			table.newRow();
			table.newHeading().cell().nest(new Font(2, true))
					.add("<BR>Request Attributes").attribute("COLSPAN", "2")
					.left();
			Enumeration a = request.getAttributeNames();
			while (a.hasMoreElements()) {
				name = (String) a.nextElement();
				table.newRow();
				table.addHeading(name + ":&nbsp;").cell()
						.attribute("VALIGN", "TOP").right();
				table.addCell("<pre>" + toString(request.getAttribute(name))
						+ "</pre>");
			}
			...
			while (a.hasMoreElements()) {
				...
				table.addHeading(name + ":&nbsp;").cell()
						.attribute("VALIGN", "TOP").right();
				table.addCell("<pre>" + toString(getInitParameter(name))
						+ "</pre>");
			}
			...
			table.newRow();
			table.newHeading().cell().nest(new Font(2, true))
					.add("<BR>Context Attributes").attribute("COLSPAN", "2")
					.left();
			...
			if (request.getContentType() != null
				...
				for (int p = 0; p < parts.length; p++) {
					...
					table.addHeading(name + ":&nbsp;").cell()
							.attribute("VALIGN", "TOP").right();
					table.addCell("<pre>" + multi.getString(parts[p])
							+ "</pre>");
				}
			}
			...
			if (res != null && res.length() > 0) {
				...
				table.newRow();
				table.addHeading("this.getClass().getClassLoader():&nbsp;")
						.cell().right();
				table.addCell(""
