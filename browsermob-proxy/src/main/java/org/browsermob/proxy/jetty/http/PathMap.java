...
/* ------------------------------------------------------------ */
/**
 * URI path map to Object. This mapping implements the path specification
 * recommended in the 2.2 Servlet API.
 * 
 ...
 * specifications. The list is separated by the characters specified in the
 * "org.mortbay.http.PathMap.separators" System property, which defaults to :
 * <P>
 * Note that this is a very different mapping to that provided by PathMap in
 * Jetty2.
 * <P>
...
public class PathMap extends HashMap implements Externalizable {
	...
	/* ------------------------------------------------------------ */
	/** The __path spec separators. */
	private static String __pathSpecSeparators = System.getProperty(
			"org.browsermob.proxy.jetty.http.PathMap.separators", ":,");

	/* ------------------------------------------------------------ */
	/**
	 * Set the path spec separator. Multiple path specification may be included
	 * in a single string if they are separated by the characters set in this
	 * string. The default value is ":," or whatever has been set by the system
	 * property org.mortbay.http.PathMap.separators
	 * 
	 * @param s
	 *            separators
	 */
	 ...
	 * 
	 * @param pathSpec
	 *            The path specification, or comma separated list of path
	 *            specifications.
	 * @param object
			...
			if (entry.getKey().equals(spec)) {
				if (spec.equals("/*"))
					...
					if (_nodefault)
						...
						_default = entry;
						_defaultSingletonList = SingletonList
								.newSingletonList(_default);
					}
				} else
					_exactMap.put(spec, entry);
			}
	...
	public static boolean match(String pathSpec, String path)
		...
		if (c == '/') {
			...
			if (pathSpec.endsWith("/*")
					&& pathSpec
							.regionMatches(0, path, 0, pathSpec.length() - 2))
				return true;
	 ...
	 * @param pathSpec
	 *            The spec of the path segment to ignore.
	 * @param path
	 *            the additional path
