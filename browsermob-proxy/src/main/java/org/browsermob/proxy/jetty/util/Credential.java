...
public abstract class Credential {
	...
	public static class Crypt extends Credential {
		 ...
		 */
		Crypt(String cooked) {
			_cooked = cooked.startsWith(Crypt.__TYPE) ? cooked.substring(__TYPE
					.length()) : cooked;
		}
	...
	public static class MD5 extends Credential {
		 ...
		 */
		MD5(String digest) {
			digest = digest.startsWith(__TYPE) ? digest.substring(__TYPE
					.length()) : digest;
			...
			try {
				...
				if (credentials instanceof Password
					...
					synchronized (__TYPE) {
						...
						__md.reset();
						__md.update(credentials.toString().getBytes(
