 ...
 * 
 * This specialization of HttpContext uses the standardized web.xml to describe
 * a web application and configure the handlers for the HttpContext.
 * 
 * If a file named web-jetty.xml or jetty-web.xml is found in the WEB-INF
 * directory it is applied to the context using the XmlConfiguration format.
 * 
 * A single WebApplicationHandler instance is used to provide security, filter,
 * sevlet and resource handling.
 * 
 * @see org.browsermob.proxy.jetty.jetty.servlet.WebApplicationHandler
 * @version $Id: WebApplicationContext.java,v 1.136 2005/10/26 08:11:04
 *          gregwilkins Exp $
 * @author Greg Wilkins (gregw)
 */
public class WebApplicationContext extends ServletHttpContext implements
		Externalizable {
	...
	/** The _configuration class names. */
	private String[] _configurationClassNames;

	/** The _resource aliases. */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the configuration class names.
	 * 
	 * @param configurationClassNames
	 *            the new configuration class names
	 */
	public void setConfigurationClassNames(String[] configurationClassNames) {
		if (null != configurationClassNames) {
			...
			System.arraycopy(configurationClassNames, 0,
					_configurationClassNames, 0, configurationClassNames.length);
		}
	}

	/* ------------------------------------------------------------ */
	/**
	 * Gets the configuration class names.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the web application handler.
	 * 
	 * @return the web application handler
	 */
	...
	private void resolveWebApp() throws IOException {
		if (_webApp == null && _war != null && _war.length() > 0) {
			...
			if (log.isDebugEnabled())
				log.debug("Try webapp=" + _webApp + ", exists="
						+ _webApp.exists() + ", directory="
						+ _webApp.isDirectory());
			...
			if (_webApp.exists()
					&& (!_webApp.isDirectory()
							|| (_extract && _webApp.getFile() == null) || (_extract
							&& _webApp.getFile() != null && !_webApp.getFile()
							.isDirectory()))) {
				// Then extract it.
	 ...
	 * exists, a new one is added to the context. This derivation of the method
	 * creates a WebApplicationHandler extension of ServletHandler.
	 * 
			...
			if (_webAppHandler == null) {
				if (getHandler(ServletHandler.class) != null)
					throw new IllegalStateException(
							"Cannot have ServletHandler in WebApplicationContext");
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.http.HttpContext#setPermissions(java.security
	...
	/* ------------------------------------------------------------ */
	/**
	 * Checks if is ignore web jetty.
	 * 
	 * @return true, if is ignore web jetty
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the ignore web jetty.
	 * 
	 * @param b
	 *            If TRUE, web-jetty.xml and jetty-web.xml configuration files
	 *            are ignored.
	 */
	...
	protected Configuration[] loadConfigurations() throws Exception {
		...
		// if this webapp does not have its own set of configurators, use the
		// defaults
		if (null == names)
			names = ((Server) getHttpServer())
		...
		if (null != names) {
			...
			for (int i = 0; i < names.length; i++) {
				configurations[i] = (Configuration) Loader.loadClass(
						WebApplicationContext.class, names[i])
						.getConstructors()[0].newInstance(nullArgs);
				if (log.isDebugEnabled()) {
		...
		try {
			...
			if (_contextListeners != null && _webAppHandler != null) {
				...
				for (int i = 0; i < LazyList.size(_contextListeners); i++) {
					try {
						((ServletContextListener) LazyList.get(
								_contextListeners, i))
	...
	/* ------------------------------------------------------------ */
	/**
	 * Stop the web application. Handlers for resource, servlet, filter and
	 * security are removed as they are recreated and configured by any
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHttpContext#destroy()
	 */
	public void destroy() {
		super.destroy();
		if (isStarted())
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.HttpContext#handle(java.lang.String,
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the defaults web.xml file. The default web.xml is used to configure
	 * all webapplications before the WEB-INF/web.xml file is applied. By
	 * default the org/browsermob/proxy/jetty/jetty/servlet/webdefault.xml
	 * resource from the org.mortbay.jetty.jar is used.
	 * 
	 * @param defaults
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the defaults descriptor.
	 * 
	 * @return the defaults descriptor
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the extract war.
	 * 
	 * @param extract
	 *            If true, a WAR is extracted to a temporary directory before
	 *            being deployed.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the extract war.
	 * 
	 * @return the extract war
	 */
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.jetty.servlet.ServletHttpContext#toString()
	 */
	public String toString() {
		String name = getDisplayName();
		return "WebApplicationContext[" + getContextPath() + ","
				+ (name == null ? _war : name) + "]";
	}

	/* ------------------------------------------------------------ */
	/**
	 * Set Resource Alias. Resource aliases map resource uri's within a context.
	 * They may optionally be used by a handler when looking for a resource.
	 * 
	 * @param alias
	 *            the alias
	 * @param uri
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the resource aliases.
	 * 
	 * @return the resource aliases
	 */
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.jetty.http.HttpContext#getResource(java.lang.String)
	 */
	 ...
	 * 
	 * @param error
	 *            A string representing an error code or a exception classname
	 * @param uriInContext
	 *            the uri in context
	 */
	 ...
	 */
	/**
	 * Base Class for WebApplicationContext Configuration. This class can be
	 * extended to customize or extend the configuration of the
	 * WebApplicationContext. If WebApplicationContext.setConfiguration is not
	...
	public static interface Configuration extends Serializable {
		 ...
		 */
		/**
		 * Configure ClassPath. This method is called before the context
		 * ClassLoader is created. Paths and libraries should be added to the
		 * context using the setClassPath, addClassPath and addClassPaths
		 * methods. The default implementation looks for WEB-INF/classes,
		 * WEB-INF/lib/*.zip and WEB-INF/lib/*.jar
		 * 
		 * @throws Exception
		 *             the exception
		 */
		 ...
		 */
		/**
		 * Configure Defaults. This method is called to intialize the context to
		 * the containers default configuration. Typically this would mean
		 * application of the webdefault.xml file. The default implementation
		 * does nothing.
		 * 
		 * @throws Exception
		 *             the exception
		 ...
		 */
		/**
		 * Configure WebApp. This method is called to apply the standard and
		 * vendor deployment descriptors. Typically this is web.xml and
		 * jetty-web.xml. The default implementation does nothing.
		 * 
		 * @throws Exception
		 *             the exception
		 */
