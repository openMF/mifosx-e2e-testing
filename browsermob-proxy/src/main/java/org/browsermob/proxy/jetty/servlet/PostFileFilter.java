 ...
 * PostFileFilter.
 * 
 * Filter to accept file content from a POST request. The translated path is
 * used as the file name to store the content from POST requests.
 * 
 * Configuration init parameters:
 * <ul>
 * <li>bufferSize - The size of the buffer used to copy content
 * <li>response - The response after the file is received:
 * <ul>
		...
		if ("POST".equalsIgnoreCase(srequest.getMethod())) {
			...
			if (file.exists()
					...
					&& (!file.getParentFile().exists()
							|| !file.getParentFile().isDirectory() || !file
							.getParentFile().canWrite())) {
				sresponse.sendError(HttpServletResponse.SC_FORBIDDEN,
						"No write permission");
				return;
			}
