...
public class XMLConfiguration implements WebApplicationContext.Configuration {
	...
	public static XmlParser webXmlParser() {
		...
		xmlParser.redirectEntity("http://www.w3.org/2001/XMLSchema.dtd",
				schemadtd);
		xmlParser.redirectEntity("-//W3C//DTD XMLSCHEMA 200102//EN", schemadtd);
		xmlParser.redirectEntity("jsp_2_0.xsd", jsp20xsd);
		xmlParser.redirectEntity("http://java.sun.com/xml/ns/j2ee/jsp_2_0.xsd",
				jsp20xsd);
		xmlParser.redirectEntity("j2ee_1_4.xsd", j2ee14xsd);
		xmlParser.redirectEntity(
				"http://java.sun.com/xml/ns/j2ee/j2ee_1_4.xsd", j2ee14xsd);
		xmlParser.redirectEntity("web-app_2_4.xsd", webapp24xsd);
		xmlParser.redirectEntity(
				"http://java.sun.com/xml/ns/j2ee/web-app_2_4.xsd", webapp24xsd);
		...
		xmlParser.redirectEntity("http://www.w3.org/2001/datatypes.dtd",
				datatypesdtd);
		xmlParser.redirectEntity("j2ee_web_services_client_1_1.xsd",
				webservice11xsd);
		xmlParser.redirectEntity(
	 ...
	 * #setWebApplicationContext(org.browsermob.proxy.jetty.jetty.servlet.
	 ...
	 */
	/**
	 * Configure ClassPath. This method is called before the context ClassLoader
	 * is created. Paths and libraries should be added to the context using the
	 * setClassPath, addClassPath and addClassPaths methods. The default
	 * implementation looks for WEB-INF/classes, WEB-INF/lib/*.zip and
	 * WEB-INF/lib/*.jar
	 * 
	 * @throws Exception
	 *             the exception
	...
	public void configureDefaults() throws Exception {
		...
		String defaultsDescriptor = getWebApplicationContext()
				.getDefaultsDescriptor();
		if (defaultsDescriptor != null && defaultsDescriptor.length() > 0) {
			Resource dftResource = Resource
					.newSystemResource(defaultsDescriptor);
			if (dftResource == null)
				dftResource = Resource.newResource(defaultsDescriptor);
			XmlParser.Node defaultConfig = xmlParser
					.parse(dftResource.getURL());
	...
	public void configureWebApp() throws Exception {
		...
		if (webInf != null && webInf.isDirectory()) {
			...
			if (!web.exists()) {
				log.info("No WEB-INF/web.xml in "
						+ getWebApplicationContext().getWAR()
	 ...
	 * within the web.xml file. It may be specialized by derived
	...
	protected void initFilter(XmlParser.Node node)
		...
		if (name == null)
			name = className;
		FilterHolder holder = getWebApplicationHandler().defineFilter(name,
				className);
	...
	protected void initFilterMapping(XmlParser.Node node) {
		...
		FilterHolder holder = (servletName != null) ? getWebApplicationHandler()
				.addFilterServletMapping(servletName, filterName, dispatcher)
				: getWebApplicationHandler().addFilterPathMapping(pathSpec,
	...
	protected void initServlet(XmlParser.Node node)
			throws ClassNotFoundException, UnavailableException, IOException,
		...
		if (className == null) {
			...
			if (jspFile != null) {
				Map.Entry entry = getWebApplicationHandler().getHolderEntry(
						jspFile);
				if (entry != null) {
		...
		if (name == null)
			name = className;
		ServletHolder holder = getWebApplicationHandler().newServletHolder(
				name, className, jspFile);
		...
		if (jspFile != null) {
			...
			if (holder.getInitParameter("classpath") == null)
				holder.setInitParameter("classpath", getWebApplicationContext()
						.getFileClassPath());
		}
		...
		while (sRefsIter.hasNext()) {
			...
			if (roleName != null && roleName.length() > 0 && roleLink != null
					&& roleLink.length() > 0) {
				if (log.isDebugEnabled())
					log.debug("link role " + roleName + " to " + roleLink
							+ " for " + this);
	...
	protected void initListener(XmlParser.Node node) {
		...
		try {
			Class listenerClass = getWebApplicationContext().loadClass(
					className);
		...
		boolean known = false;
		try {
			getWebApplicationContext().addEventListener(
		...
		try {
			getWebApplicationHandler().addEventListener(
					(EventListener) listener);
			known = true;
		} catch (Exception e) {
			LogSupport.ignore(log, e);
	...
	/* ------------------------------------------------------------ */
	/**
	 * Inits the locale encoding list.
	 * 
	 * @param node
	 *            the node
	 */
	...
	protected void initJspConfig(XmlParser.Node node) {
		for (int i = 0; i < node.size(); i++) {
			Object o = node.get(i);
			if (o instanceof XmlParser.Node
					&& "taglib".equals(((XmlParser.Node) o).getTag()))
	...
	protected void initSecurityConstraint(XmlParser.Node node) {
		try {
			...
			if (auths != null) {
				...
				while (iter.hasNext()) {
					String role = ((XmlParser.Node) iter.next()).toString(
							false, true);
					scBase.addRole(role);
				}
			}
			...
			while (iter.hasNext()) {
				...
				String name = collection.getString("web-resource-name", false,
						true);
				SecurityConstraint sc = (SecurityConstraint) scBase.clone();
				sc.setName(name);
				Iterator iter2 = collection.iterator("http-method");
				while (iter2.hasNext())
					sc.addMethod(((XmlParser.Node) iter2.next()).toString(
							false, true));
				iter2 = collection.iterator("url-pattern");
				while (iter2.hasNext()) {
					String url = ((XmlParser.Node) iter2.next()).toString(
							false, true);
	...
	protected void initLoginConfig(XmlParser.Node node) {
		...
		if (formConfig != null) {
			if (_formAuthenticator == null)
				...
				if (loginPage != null)
					_formAuthenticator.setLoginPage(loginPage.toString(false,
							true));
