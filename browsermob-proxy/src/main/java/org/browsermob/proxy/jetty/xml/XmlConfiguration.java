...
/* ------------------------------------------------------------ */
/**
 * Configure Objects from XML. This class reads an XML file conforming to the
 * configure.dtd DTD and uses it to configure and object by calling set, put or
 * other methods on the object.
 * 
 * @version $Id: XmlConfiguration.java,v 1.28 2005/08/13 08:12:14 gregwilkins
 *          Exp $
...
public class XmlConfiguration {
	...
	/** The __primitives. */
	private static Class[] __primitives = { Boolean.TYPE, Character.TYPE,
			Byte.TYPE, Short.TYPE, Integer.TYPE, Long.TYPE, Float.TYPE,
			Double.TYPE, Void.TYPE };

	/** The __primitive holders. */
	private static Class[] __primitiveHolders = { Boolean.class,
	...
	private synchronized static void initParser() throws IOException {
		...
		__parser.redirectEntity("http://jetty.jetty.org/configure_1_3.dtd",
				config13URL);
		__parser.redirectEntity("http://jetty.jetty.org/configure.dtd",
				config13URL);
		__parser.redirectEntity(
				"-//Mort Bay Consulting//DTD Configure 1.3//EN", config13URL);
		__parser.redirectEntity("-//Mort Bay Consulting//DTD Configure//EN",
				config13URL);
		...
		__parser.redirectEntity("http://jetty.jetty.org/configure_1_2.dtd",
				config12URL);
		__parser.redirectEntity(
				"-//Mort Bay Consulting//DTD Configure 1.2//EN", config12URL);
		...
		__parser.redirectEntity("http://jetty.jetty.org/configure_1_1.dtd",
				config11URL);
		__parser.redirectEntity(
				"-//Mort Bay Consulting//DTD Configure 1.1//EN", config11URL);
		...
		__parser.redirectEntity("http://jetty.jetty.org/configure_1_0.dtd",
				config10URL);
		__parser.redirectEntity(
				"-//Mort Bay Consulting//DTD Configure 1.0//EN", config10URL);
	}
	 ...
	 * 
	 * @param configuration
	 *            String of XML configuration commands excluding the normal XML
	 *            preamble. The String should start with a " <Configure ...."
	 *            element.
	 * @throws SAXException
	 *             the sAX exception
	 * @throws IOException
	...
	public XmlConfiguration(String configuration) throws SAXException,
			IOException {
		initParser();
		configuration = "<?xml version=\"1.0\"  encoding=\"ISO-8859-1\"?>\n<!DOCTYPE Configure PUBLIC \"-//Mort Bay Consulting//DTD Configure 1.2//EN\" \"http://jetty.jetty.org/configure_1_2.dtd\">"
				+ configuration;
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public XmlConfiguration(InputStream configuration) throws SAXException,
			IOException {
		initParser();
	...
	/* ------------------------------------------------------------ */
	/**
	 * Configure an object. If the object is of the approprate class, the XML
	 * configuration script is applied to the object.
	 * 
	 * @param obj
	 ...
	 */
	public void configure(Object obj) throws ClassNotFoundException,
		...
		if (oClass != null) {
			if (obj != null && !oClass.isInstance(obj))
				throw new IllegalArgumentException("Object is not of type "
						+ oClass);
			if (obj == null)
	 ...
	 * and Call elements to the current object. @param obj @param cfg @param i
	 ...
	 * node @return @exception NoSuchMethodException @exception
	 ...
	 * @param obj
	 * 
	 * @param node @return @exception NoSuchMethodException @exception
		...
		if (type != null) {
			...
			if (aClass == null) {
				if ("String".equals(type))
					...
					aClass = org.browsermob.proxy.jetty.util.InetAddrPort.class;
				else
	...
	private Object value(Object obj, XmlParser.Node node)
		...
		if ("InetAddrPort".equals(type)
				|| "org.browsermob.proxy.jetty.util.InetAddrPort".equals(type)) {
	...
	public static void main(String[] arg) {
		try {
			for (int i = 0; i < arg.length; i++)
				new XmlConfiguration(Resource.newResource(arg[i]).getURL())
						.newInstance();
		} catch (Exception e) {
