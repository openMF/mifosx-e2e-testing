...
/* ------------------------------------------------------------ */
/**
 * Model MBean Implementation. This implementation of the JMX Model MBean API is
 * designed to allow easy creation of Model MBeans. From minimal descriptions of
 * operations and attributes, reflection is used to determine the full signature
 * and ResourceBundles are used to determine other meta data.
 * 
 * This class is normally used in one of the following patterns:
 * <UL>
 * <LI>As a base class for a real MBean that contains the actual attributes and
 * operations of the MBean. Such an Object is only usable in a JMX environment.
 * <LI>As a proxy MBean to another non-JMX object. The attributes and operations
 * of the proxied object are defined in the MBean. This pattern is used when an
 * existing non-JMX objects API is to be exposed as an MBean.
 * <LI>As a base class for a proxy MBean. The attributes and oepration of the
 * MBean are implemented by the derived class but delegate to one or more other
 * objects. This pattern is used if existing objects are to be managed by JMX,
 * but a new management API needs to be defined.
 * </UL>
 * 
...
public class ModelMBeanImpl implements ModelMBean, MBeanRegistration {
	...
	/* ------------------------------------------------------------ */
	/**
	 * MBean Constructor. No proxy object is defined. Attributes and operations
	 * are defined on this instance.
	 */
	 ...
	 * 
	 * @param proxyObject
	 *            The actual object on which attributes and operations are to be
	 *            defined and called.
	 */
	 ...
	 * 
	 * @see
	 * javax.management.modelmbean.ModelMBean#setManagedResource(java.lang.Object
	 * , java.lang.String)
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Define the Managed Resource. This method is called the first time
	 * setManagedResource is called with a non-null object. It should be
	 * implemented by a derived ModelMBean to define the attributes and
	 * operations after an initial object has been set.
	 */
	 ...
	 * 
	 * @param info
	 *            the new model m bean info
	 * @throws MBeanException
	 *             the m bean exception
	 * @throws RuntimeOperationsException
	 *             the runtime operations exception
	 */
	 ...
	 * with a call to findDescription with the attribute name.
	 * 
	 * @param name
	 *            The name of the attribute. Normal java bean capitlization is
	 *            enforced on this name.
	 */
	...
	public synchronized void defineAttribute(String name, boolean writable,
		...
		for (int m = 0; m < methods.length; m++) {
			...
			if (methods[m].getName().equals("get" + uName)
				...
				if (getter != null)
					throw new IllegalArgumentException(
							"Multiple getters for attr " + name);
				getter = methods[m];
				if (type != null && !type.equals(methods[m].getReturnType()))
					throw new IllegalArgumentException(
							"Type conflict for attr " + name);
			...
			if (writable && methods[m].getName().equals("set" + uName)
				...
				if (setter != null)
					throw new IllegalArgumentException(
							"Multiple setters for attr " + name);
				setter = methods[m];
				if (type != null
		...
		if (getter == null && setter == null)
			throw new IllegalArgumentException(
					"No getter or setters found for " + name);
		...
		try {
			...
			_attributes.add(new ModelMBeanAttributeInfo(name,
					findDescription(name), getter, setter));
		} catch (Exception e) {
			log.warn(LogSupport.EXCEPTION, e);
			throw new IllegalArgumentException(e.toString());
		}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Define an attribute. Explicit definition of an attribute. Reflection is
	 * used to locate the actual getter and setter methods.
	 * 
	 * @param attrInfo
		...
		try {
			...
			if (type == null)
				type = Thread.currentThread().getContextClassLoader()
						.loadClass(attrInfo.getType());

			Method getter = null;
			Method setter = null;

			if (attrInfo.isReadable())
				getter = oClass.getMethod((attrInfo.isIs() ? "is" : "get")
						+ uName, (java.lang.Class[]) null);
	...
	/* ------------------------------------------------------------ */
	/**
	 * Define an operation on the managed object. Defines an operation with no
	 * parameters. Refection is used to determine the return type and the
	 * description is found with a call to findDescription on "name()".
	 * 
	 * @param name
	 ...
	 * return type. The description of the method is found with a call to
	 * findDescription on "name(signature)". The name and description of each
	 * parameter is found with a call to findDescription with
	 * "name(partialSignature", the returned description is for the last
	 * parameter of the partial signature and is assumed to start with the
	 * parameter name, followed by a colon.
	 * 
	 * @param name
	 ...
	 *            Impact as defined in MBeanOperationInfo
	 */
	public synchronized void defineOperation(String name, String[] signature,
			int impact) {
	...
	public synchronized void defineOperation(String name, String[] signature,
		...
		try {
			...
			for (int i = 0; i < signature.length; i++) {
				...
				types[i] = type;
				signature[i] = type.isPrimitive() ? TypeUtil.toName(type)
						: signature[i];
			...
			for (int i = 0; i < signature.length; i++) {
				...
				pInfo[i] = new MBeanParameterInfo(description.substring(0,
						colon).trim(), signature[i], description.substring(
						colon + 1).trim());
			}
			...
			_operations.add(new ModelMBeanOperationInfo(name,
					findDescription(methodKey), pInfo, returnClass
							.isPrimitive() ? TypeUtil.toName(returnClass)
							: (returnClass.getName()), impact));
		} catch (Exception e) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Define an operation. Explicit definition of an operation. Reflection is
	 * used to locate method called.
	 * 
	 * @param opInfo
	 *            the op info
	 */
	...
	public synchronized MBeanInfo getMBeanInfo() {
		...
		if (_dirty) {
			...
			ModelMBeanNotificationInfo[] notifications = (ModelMBeanNotificationInfo[]) _notifications
					.toArray(new ModelMBeanNotificationInfo[_notifications
							.size()]);

			_beanInfo = new ModelMBeanInfoSupport(_object.getClass().getName(),
					findDescription(null), attributes, null, operations,
					notifications);
		}
	...
	public AttributeList setAttributes(AttributeList attrs) {
		...
		while (iter.hasNext()) {
			try {
				...
				setAttribute(attr);
				results.add(new Attribute(attr.getName(), getAttribute(attr
						.getName())));
			} catch (Exception e) {
				log.warn(LogSupport.EXCEPTION, e);
			}
	 ...
	 * (javax.management.NotificationListener, java.lang.String,
	 * java.lang.Object)
	 */
	...
	public void removeAttributeChangeNotificationListener(
			NotificationListener listener, String name) throws MBeanException,
	 ...
	 * 
	 * @see javax.management.modelmbean.ModelMBeanNotificationBroadcaster#
	 * sendAttributeChangeNotification(javax.management.Attribute,
	...
	public void sendAttributeChangeNotification(
			AttributeChangeNotification notify) throws MBeanException,
			RuntimeOperationsException {
		log.debug("sendAttributeChangeNotification");
	}
	 ...
	 */
	public void sendNotification(String notify) throws MBeanException,
			RuntimeOperationsException {
	 ...
	 */
	public void sendNotification(Notification notify) throws MBeanException,
			RuntimeOperationsException {
		log.debug("sendNotification");
	}
	...
	/* ------------------------------------------------------------ */
	/*
	 * Find MBean descriptions. MBean descriptions are searched for in
	 * ResourceBundles. Bundles are looked for in a mbean.property files within
	 * each package of the MBean class inheritance hierachy. Once a bundle is
	 * found, the key is added to object names in the following order: fully
	 * qualied managed resource class name, tail managed resource class name,
	 * tail mbean class name. The string "MBean" is stripped from the tail of
	 * any name. <P>For example, if the class a.b.C is managed by a MBean
	 * p.q.RMBean which is derived from p.SMBean, then the seach order for a key
	 * x is as follows:<PRE> bundle: p.q.mbean name: a.b.C.x bundle: p.q.mbean
	 * name: C.x bundle: p.q.mbean name: R.x bundle: p.mbean name: a.b.C.x
	 * bundle: p.mbean name: C.x bundle: p.mbean name: S.x </PRE> <P>The
	 * convention used for keys passed to this method are:<PRE> null or empty -
	 * Object description xxx - Attribute xxx description xxx() - Simple
	 * operation xxx description xxx(type,..) - Operation xxx with signature
	 * desciption xxx(type,..)[n] - Param n of operation xxx description </PRE>
	 * 
	 * @param key
	 * 
	...
	private String findDescription(String key) {
		...
		if (objectNames[0].indexOf(".") >= 0)
			objectNames[1] = objectNames[0].substring(objectNames[0]
					.lastIndexOf(".") + 1);
		...
		while (lookIn != null) {
			...
			String resource = (pkg == null ? "mbean"
					: (pkg.replace('.', '/') + "/mbean"));
			if (log.isTraceEnabled())
			...
			try {
				ResourceBundle bundle = ResourceBundle.getBundle(resource,
						Locale.getDefault(), _object.getClass()
								.getClassLoader());
	...
	/* ------------------------------------------------------------ */
	/**
	 * Create a new ObjectName. Return a new object name. The default
	 * implementation is the results of uniqueObjectName(baseObjectName), if
	 * baseObjectName is not set, then the results of
	 * uniqueObjectName(defaultDomain+":");
	 * 
	 * @param server
	 *            the server
	 * @return The Object name
	...
	/* ------------------------------------------------------------ */
	/**
	 * Pre registration notification. If this method is specialized by a derived
	 * class that may set the objectName, then it should call this
	 * implementation with the new objectName.
	 * 
	 * @param server
	 *            the server
	 * @param oName
	...
	/* ------------------------------------------------------------ */
	/**
	 * Post Deregister. This implementation destroys this MBean and it cannot be
	 * used again.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add an id clause to a JMX object name. Used to make unique objectnames
	 * when there are no other distinguishing attributes. If the passed object
	 * name ends with '=', just a unique ID is added. Otherwise and classname=
	 * clause is added.
	 * 
	 * @param server
	 *            the server
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get Component MBeans. Creates, registers and deregisters MBeans for an
	 * array of components. On each call the passed map is used to determine
	 * components that have already been registers and those that need to be
	 * deregistered.
	 * 
	 * @param components
	 *            the components.
	 * @param map
	 *            A map of previously registered components to object name. If
	 *            null is passed, a default map for the mbean is used.
						...
						try {
							if (mbean instanceof ModelMBeanImpl) {
								((ModelMBeanImpl) mbean)
										.setBaseObjectName(getObjectName()
												.toString());
								on = getMBeanServer()
								...
								on = uniqueObjectName(getMBeanServer(),
										components[i], getObjectName()
												.toString());
								on = getMBeanServer().registerMBean(mbean, on)
										.getObjectName();
							}
			...
			for (; d-- > 0;) {
				try {
					getMBeanServer().unregisterMBean(
							(ObjectName) map.remove(to_delete[d]));
				} catch (Exception e) {
					log.warn(LogSupport.EXCEPTION, e);
				}
			}
