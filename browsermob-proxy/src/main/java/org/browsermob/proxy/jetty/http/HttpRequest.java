...
/* ------------------------------------------------------------ */
/**
 * HTTP Request. This class manages the headers, trailers and content streams of
 * a HTTP request. It can be used for receiving or generating requests.
 * <P>
 * This class is not synchronized. It should be explicitly synchronized if it is
...
public class HttpRequest extends HttpMessage {
	 ...
	 * Request METHODS.
	 */
	public static final String __GET = "GET", __POST = "POST", __HEAD = "HEAD",
			__PUT = "PUT", __OPTIONS = "OPTIONS", __DELETE = "DELETE",
			__TRACE = "TRACE", __CONNECT = "CONNECT", __MOVE = "MOVE";

	/* ------------------------------------------------------------ */
	/**
	 * Max size of the form content. Limits the size of the data a client can
	 * push at the server. Set via the
	 * org.mortbay.http.HttpRequest.maxContentSize system property.
	 */
	public static int __maxFormContentSize = Integer.getInteger(
			"org.browsermob.proxy.jetty.http.HttpRequest.maxFormContentSize",
			200000).intValue();
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the HTTP Response. Get the HTTP Response associated with this
	 * request.
	 * 
	 ...
	 * 
	 * @return True if the request has been set to handled or the associated
	...
	/* -------------------------------------------------------------- */
	/**
	 * Get the HTTP method for this request. Returns the method with which the
	 * request was made. The returned value can be "GET", "HEAD", "POST", or an
	 * extension method. Same as the CGI variable REQUEST_METHOD.
	 * 
	 * @return The method
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.jetty.http.HttpMessage#getVersion()
	 */
	 ...
	 * URL contains a protocol, server name, port number, and, but it does not
	 * include a path.
	 * <p>
	 * Because this method returns a <code>StringBuffer</code>, not a string,
	 * you can modify the URL easily, for example, to append path and query
	 * parameters.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the request Scheme. The scheme is obtained from an absolute URI. If
	 * the URI in the request is not absolute, then the connections default
	 ...
	 * 
	 * @return True if this request came over an confidential channel such as
	 *         SSL.
	 */
	 ...
	 * 
	 * @return The host name obtained from an absolute URI, the HTTP header
	 *         field, the requests connection or the local host name.
	 */
	 ...
	 * the HTTP Host header field, the connection or the default.
	 * 
	...
	void decodeRequestLine(char[] buf, int len) throws IOException {
		...
		try {
			...
			LogSupport.ignore(log, e);
			throw new HttpException(HttpResponse.__400_Bad_Request, new String(
					buf, s3, e3 - s3 + 1));
		}
	}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Force a removeField. This call ignores the message state and forces a
	 * field to be removed from the request. It is required for the handling of
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the acceptable transfer encodings. The TE field is used to construct
	 * a list of acceptable extension transfer codings in quality order. An
	 * empty list implies that only "chunked" is acceptable. A null list implies
	 * that no transfer coding can be applied.
	 * 
	 * If the "trailer" coding is found in the TE field, then message trailers
	 * are enabled in any linked response.
	 * 
	...
	public List getAcceptableTransferCodings() {
		...
		if (tenum != null) {
			...
			if (size > 0) {
				...
				while (iter.hasNext()) {
					String coding = StringUtil.asciiToLowerCase(HttpFields
							.valueParameters(iter.next().toString(), null));
	...
	private void extractParameters() {
		...
		if (_state == __MSG_RECEIVED) {
			...
			if (content_type != null && content_type.length() > 0) {
				...
				if (HttpFields.__WwwFormUrlEncode
					...
					if (content_length == 0)
						...
						try {
							...
							if (bout.size() == __maxFormContentSize
									&& in.available() > 0)
								throw new IllegalStateException(
										"Form too large");
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the parameter string array map.
	 * 
	...
	public Cookie[] getCookies() {
		...
		try {
			...
			while (enm.hasMoreElements()) {
				...
				// Parse the header
				QuotedStringTokenizer tok = new QuotedStringTokenizer(hdr,
						",;", false, false);
				tok.setSingle(false);
					...
					try {
						...
						if (n.startsWith("$")) {
							if ("$version".equalsIgnoreCase(n))
								version = Integer
										.parseInt(QuotedStringTokenizer
												.unquoteDouble(v));
