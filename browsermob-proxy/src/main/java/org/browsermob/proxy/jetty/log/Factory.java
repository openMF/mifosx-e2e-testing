 ...
 * unless an alternate Log implementation has been set as an attribute keyed by
 * the classname or other name of the log. If the name of the attibute ends with
 * '.*' it is assumed to be a name prefix match. Attributes with string values
 * are treated as references to other attributes.
 * 
 * This class needs to be configured in the META-INF/services directory (see
 * build.xml for example) for automatic discovery. Or it can be configured with
 * the system property:
...
public class Factory extends org.apache.commons.logging.LogFactory {
	 ...
	 * 
	 * @see
	 * org.apache.commons.logging.LogFactory#removeAttribute(java.lang.String)
	 */
