...
package org.browsermob.proxy.jetty.stop;
...
/*-------------------------------------------*/
/**
 * Main stop class. This class is intended to be the main class listed in the
 * MANIFEST.MF of the stop.jar archive. It allows an application started with
 * the command "java -jar start.jar" to be stopped.
 * 
...
public class Main {
	...
	private String _config = System.getProperty("START",
			"org/browsermob/proxy/jetty/start/start.config");
	...
	void stop() {
		try {
			if (_port <= 0)
				System.err
						.println("START.PORT system property must be specified");
			if (_key == null) {
				_key = "";
