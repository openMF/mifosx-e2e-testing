...
public class ProxyHandler extends AbstractHttpHandler {
	 ...
	 * 
	 * @return Array of hostnames and IPs that are proxied, or an empty array if
	 ...
	 * 
	 * @param hosts
	 *            Array of hostnames and IPs that are proxied, or null if all
	 *            hosts are proxied.
	 */
		...
		try {
			...
			if (isForbidden(HttpMessage.__SSL_SCHEME, addrPort.getHost(),
				...
				HttpTunnel tunnel = newHttpTunnel(request, response,
						addrPort.getInetAddress(), addrPort.getPort(),
						timeoutMs);
	...
	protected HttpTunnel newHttpTunnel(HttpRequest request,
		...
		try {
			...
			if (chained_proxy_host == null) {
				...
				int chained_proxy_port = Integer.getInteger("http.proxyPort",
						8888).intValue();

				Socket chain_socket = new Socket(chained_proxy_host,
						chained_proxy_port);
				...
				byte[] connect = request
						.toString()
						.getBytes(
								org.browsermob.proxy.jetty.util.StringUtil.__ISO_8859_1);
				...
				if (space0 > 0 && space0 + 1 < chain_response_line.length()) {
					...
					if (space1 > space0) {
						int code = Integer.parseInt(chain_response_line
								.substring(space0 + 1, space1));
						...
						if (code >= 200 && code < 300) {
							...
							while (iter.hasMoreElements()) {
								...
								if (!_DontProxyHeaders.containsKey(name)) {
									Enumeration values = chain_response
											.getValues(name);
	...
	protected void customizeConnection(String pathInContext, String pathParams,
			HttpRequest request, Socket socket) throws IOException {
	}
	 ...
	 */
	protected void customizeConnection(String pathInContext, String pathParams,
			HttpRequest request, URLConnection connection) throws IOException {
	}
