...
import org.browsermob.proxy.jetty.http.ResourceCache.ResourceMetaData;
import org.browsermob.proxy.jetty.http.handler.ErrorPageHandler;
...
/* ------------------------------------------------------------ */
/**
 * Context for a collection of HttpHandlers. HTTP Context provides an ordered
 * container for HttpHandlers that share the same path prefix, filebase,
 * resourcebase and/or classpath.
 * <p>
 * A HttpContext is analagous to a ServletContext in the Servlet API, except
 * that it may contain other types of handler other than servlets.
 * <p>
 * A ClassLoader is created for the context and it uses
 * Thread.currentThread().getContextClassLoader(); as it's parent loader. The
 * class loader is initialized during start(), when a derived context calls
 * initClassLoader() or on the first call to loadClass()
 * <p>
 * 
 * <B>Note. that order is important when configuring a HttpContext. For example,
 * if resource serving is enabled before servlets, then resources take
 * priority.</B>
 * 
 * @see HttpServer
 * @see HttpHandler
 * @see org.browsermob.proxy.jetty.jetty.servlet.ServletHttpContext
 * @version $Id: HttpContext.java,v 1.136 2006/02/21 09:47:43 gregwilkins Exp $
 * @author Greg Wilkins (gregw)
 */
public class HttpContext extends Container implements LifeCycle, HttpHandler,
		EventProvider, Serializable {
	...
	/* ------------------------------------------------------------ */
	/**
	 * File class path attribute. If this name is set as a context init
	 * parameter, then the attribute name given will be used to set the file
	 * classpath for the context as a context attribute.
	 */
	public final static String __fileClassPathAttr = "org.browsermob.proxy.jetty.http.HttpContext.FileClassPathAttribute";

	/** The Constant __ErrorHandler. */
	public final static String __ErrorHandler = "org.browsermob.proxy.jetty.http.ErrorHandler";
	...
	/** The _class loader java2 compliant. */
	...
	/** The _system classes. */
	private String[] _systemClasses = new String[] { "java.", "javax.servlet.",
			"javax.xml.", "org.browsermob.proxy.jetty.", "org.xml.",
			"org.w3c.", "org.apache.commons.logging." };

	/** The _server classes. */
	private String[] _serverClasses = new String[] {
			"-org.browsermob.proxy.jetty.http.PathMap",
			"-org.browsermob.proxy.jetty.jetty.servlet.Invoker",
			"-org.browsermob.proxy.jetty.jetty.servlet.JSR154Filter",
			"-org.browsermob.proxy.jetty.jetty.servlet.Default",
			"org.browsermob.proxy.jetty.jetty.Server",
			"org.browsermob.proxy.jetty.http.",
			"org.browsermob.proxy.jetty.start.",
			"org.browsermob.proxy.jetty.stop." };
	...
	/** The _welcomes. */
	private String[] _welcomes = { "welcome.html", "index.html", "index.htm",
			"index.jsp" };
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the ThreadLocal HttpConnection. Get the HttpConnection for current
	 * thread, if any. This method is not static in order to control access.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the stop gracefully.
	 * 
	 * @param graceful
	 *            the new stop gracefully
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the stop gracefully.
	 * 
	 * @return the stop gracefully
	 */
	...
	public static String canonicalContextPathSpec(String contextPathSpec) {
		// check context path
		if (contextPathSpec == null || contextPathSpec.indexOf(',') >= 0
				|| contextPathSpec.startsWith("*"))
			throw new IllegalArgumentException("Illegal context spec:"
					+ contextPathSpec);
	...
	public void setContextPath(String contextPathSpec) {
		...
		if (contextPathSpec.length() > 1)
			_contextPath = contextPathSpec.substring(0,
					contextPathSpec.length() - 2);
		else
			_contextPath = "/";
	 ...
	 * 
	 * @param hostname
	 *            A hostname. A null host name means any hostname is acceptable.
	 ...
	 * host header or fully qualified URL will be passed to that context with a
	 * virtual host name. A context with no virtual host names or a null virtual
	 * host name is available to all requests that are not served by a context
	 * with a matching virtual host name.
	 * 
	 * @param hosts
	 *            Array of virtual hosts that this context responds to. A null
	 *            host name or null/empty array means any hostname is
	 *            acceptable. Host names may String representation of IP
	 *            addresses.
	 */
	 ...
	 * with a matching virtual host name.
	 * 
	 * @return Array of virtual hosts that this context responds to. A null host
	 *         name or empty array means any hostname is acceptable. Host names
	 *         may be String representation of IP addresses.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the hosts for the context. Set the real hosts that this context will
	 * accept requests for. If not null or empty, then only requests from
	 * HttpListeners for hosts in this array are accepted by this context.
	 * Unlike virutal hosts, this value is not used by HttpServer for matching a
	 * request to a context.
	 * 
	 * @param hosts
	 *            the new hosts
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set system classes. System classes cannot be overriden by context
	 * classloaders.
	 * 
	 * @param classes
	 *            array of classname Strings. Names ending with '.' are treated
	 *            as package names. Names starting with '-' are treated as
	 *            negative matches and must be listed before any enclosing
	 *            packages.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get system classes. System classes cannot be overriden by context
	 * classloaders.
	 * 
	 * @return array of classname Strings. Names ending with '.' are treated as
	 *         package names. Names starting with '-' are treated as negative
	 *         matches and must be listed before any enclosing packages. Null if
	 *         not set.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set system classes. Servers classes cannot be seen by context
	 * classloaders.
	 * 
	 * @param classes
	 *            array of classname Strings. Names ending with '.' are treated
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get system classes. System classes cannot be seen by context
	 * classloaders.
	 * 
	 * @return array of classname Strings. Names ending with '.' are treated as
	 *         package names. Names starting with '-' are treated as negative
	 ...
	 * 
	 * @param i
	 *            The position in the handler list
	 * @param handler
	 *            The handler.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set context init parameter. Init Parameters differ from attributes as
	 * they can only have string values, servlets cannot set them and they do
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the welcome files.
	 * 
	 * @param welcomes
	 *            the new welcome files
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the context classpath. This method only returns the paths that have
	 * been set for this context and does not include any paths from a parent or
	 * the system classloader. Note that this may not be a legal javac
	 * classpath.
	 * 
	 * @return a comma or ';' separated list of class resources. These may be
	 *         jar files, directories or URLs to jars or directories.
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the file classpath of the context. This method makes a best effort to
	 * return a complete file classpath for the context. It is obtained by
	 * walking the classloader hierarchy and looking for URLClassLoaders. The
	 * system property java.class.path is also checked for file elements not
	 * already found in the loader hierarchy.
	 * 
	 * @return Path of files and directories for loading classes.
	 * @throws IllegalStateException
	 *             HttpContext.initClassLoader has not been called.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the class path for the context. A class path is only required for a
	 * context if it uses classes that are not in the system class path.
	 * 
	 * @param classPath
	 *            a comma or ';' separated list of class resources. These may be
	 *            jar files, directories or URLs to jars or directories.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add the class path element to the context. A class path is only required
	 * for a context if it uses classes that are not in the system class path.
	 * 
	 * @param classPath
	 *            a comma or ';' separated list of class resources. These may be
	 *            jar files, directories or URLs to jars or directories.
	 ...
	 *         specification and delegate all loads to the parent classloader.
	 *         If false, the context classloader only delegate loads for system
	 *         classes or classes that it can't find itself.
	 */
	 ...
	 *            specification and delegate all loads to the parent
	 *            classloader. If false, the context classloader only delegate
	 *            loads for system classes or classes that it can't find itself.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set temporary directory for context. The javax.servlet.context.tempdir
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get Context temporary directory. A tempory directory is generated if it
	 * has not been set. The "javax.servlet.context.tempdir" attribute is
	 * consulted and if not set, the host, port and context are used to generate
	 * a directory within the JVMs temporary directory.
	 * 
	...
	public File getTempDirectory() {
		...
		if (t != null && (t instanceof String)) {
			try {
				...
				if (_tmpDir.isDirectory() && _tmpDir.canWrite()) {
					if (log.isDebugEnabled())
						log.debug("Converted to File " + _tmpDir + " for "
								+ this);
					setAttribute("javax.servlet.context.tempdir", _tmpDir);
					return _tmpDir;
				}
		...
		// No tempdir so look for a WEB-INF/work directory to use as tempDir
		// base
		File work = null;
		try {
		...
		try {
			...
			String vhost = null;
			for (int h = 0; vhost == null && _vhosts != null
					&& h < _vhosts.size(); h++)
				vhost = (String) _vhosts.get(h);
			String host = httpListener.getHost();
			String temp = "Jetty_" + (host == null ? "" : host) + "_"
					+ httpListener.getPort() + "_"
			...
			if (work != null)
				...
				if (_tmpDir.exists()) {
					if (log.isDebugEnabled())
						log.debug("Delete existing temp dir " + _tmpDir
								+ " for " + this);
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the classloader. If no classloader has been set and the context has
	 * been loaded normally, then null is returned. If no classloader has been
	 * set and the context was loaded from a classloader, that loader is
	 * returned. If a classloader has been set and no classpath has been set
	 * then the set classloader is returned. If a classloader and a classpath
	 * has been set, then a new URLClassloader initialized on the classpath with
	 * the set loader as a partent is return.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set Parent ClassLoader. By default the parent loader is the thread
	 * context classloader of the thread that calls initClassLoader. If
	 * setClassLoader is called, then the parent is ignored.
	 * 
	 * @param loader
	 *            The class loader to use for the parent loader of the context
	 *            classloader.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Initialize the context classloader. Initialize the context classloader
	 * with the current parameters. Any attempts to change the classpath after
	 * this call will result in a IllegalStateException
	 * 
	 * @param forceContextLoader
	 *            If true, a ContextLoader is always if no loader has been set.
	...
	protected void initClassLoader(boolean forceContextLoader)
		...
		if (_loader == null) {
			...
			if (log.isDebugEnabled())
				log.debug("Init classloader from " + _classPath + ", " + parent
						+ " for " + this);

			if (forceContextLoader || _classPath != null
					|| _permissions != null) {
				ContextLoader loader = new ContextLoader(this, _classPath,
						parent, _permissions);
	 ...
	 * 
	 * @param realmName
	 *            The name to use to retrieve the actual realm from the
	 *            HttpServer
	 */
		...
		if (scss != null && scss.size() > 0) {
			...
			loop: for (int m = 0; m < scss.size(); m++) {
				...
				for (int c = 0; c < LazyList.size(scs); c++) {
					SecurityConstraint sc = (SecurityConstraint) LazyList.get(
							scs, c);
			...
			return SecurityConstraint.check(LazyList.getList(constraints),
					_authenticator, realm, pathInContext, request, response);
		}
	 ...
	 * 
	 * @param b
	 *            if true a /context request will be redirected to /context/ if
	...
	/* ------------------------------------------------------------ */
	/**
	 * Checks if is redirect null path.
	 * 
	 * @return True if a /context request is redirected to /context/ if there is
	 *         not path in the context.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the permissions to be used for this context. The collection of
	 * permissions set here are used for all classes loaded by this context.
	 * This is simpler that creating a security policy file, as not all code
	 * sources may be statically known.
	 * 
	 * @param permissions
	 *            the new permissions
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Add a permission to this context. The collection of permissions set here
	 * are used for all classes loaded by this context. This is simpler that
	 * creating a security policy file, as not all code sources may be
	 * statically known.
	 * 
	 * @param permission
	 *            the permission
	...
	/* ------------------------------------------------------------ */
	/**
	 * Handler request. Determine the path within the context and then call
	...
	public void handle(HttpRequest request, HttpResponse response)
		...
		if (_redirectNullPath
				&& (pathInContext == null || pathInContext.length() == 0)) {
			...
			if (log.isDebugEnabled())
				log.debug(this + " consumed all of path " + request.getPath()
	...
	/* ------------------------------------------------------------ */
	/**
	 * Enter the context scope. This method is called (by handle or servlet
	 * dispatchers) to indicate that request handling is entering the scope of
	 * this context. The opaque scope object returned, should be passed to the
	 * leaveContextScope method.
	 * 
	 * @param request
	 *            the request
	 * @param response
	...
	/* ------------------------------------------------------------ */
	/**
	 * Leave the context scope. This method is called (by handle or servlet
	 * dispatchers) to indicate that request handling is leaveing the scope of
	 * this context. The opaque scope object returned by enterContextScope
	 * should be passed in.
	 * 
	 * @param request
	 *            the request
	 ...
	 *            the old scope
	 */
	public void leaveContextScope(HttpRequest request, HttpResponse response,
			Object oldScope) {
		if (oldScope == null) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the http context name.
	 * 
	 * @return the http context name
	 */
	public String getHttpContextName() {
		if (_contextName == null)
			_contextName = (_vhosts.size() > 1 ? (_vhosts.toString() + ":")
					: "") + _contextPath;
		return _contextName;
	}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Sets the http context name.
	 * 
	 * @param s
	 *            the new http context name
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Start the handlers. This is called by start after the classloader has
	 * been initialized and set as the thread context loader. It may be
	 * specialized to provide custom handling before any handlers are started.
	 * 
	 * @throws Exception
	 *             the exception
	 */
	 ...
	 * 
	 * @param graceful
	 *            If true and statistics are on, then this method will wait for
	 *            requestsActive to go to zero before calling stop()
	...
	public void stop(boolean graceful) throws InterruptedException {
		...
		try {
			...
			while (graceful && _statsOn && _requestsActive > 0
					&& _httpServer != null)
				try {
					Thread.sleep(100);
				} catch (InterruptedException e) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Send an error response. This method may be specialized to provide
	 * alternative error handling for errors generated by the container. The
	 * default implemenation calls HttpResponse.sendError
	 * 
	 * @param response
	...
	/* ------------------------------------------------------------ */
	/**
	 * Send an error response. This method obtains the responses context and
	 * call sendError for context specific error handling.
	 * 
	 * @param response
	 *            the response to send
	 * @param code
	 ...
	 *             Problem sending response.
	 */
	public static void sendContextError(HttpResponse response, int code,
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the stats on ms.
	 * 
	 * @return the stats on ms
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests active.
	 * 
	 * @return Number of requests currently active. Undefined if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the requests active max.
	 * 
	 * @return Maximum number of active requests since statsReset() called.
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the responses1xx.
	 * 
	 * @return Get the number of responses with a 2xx status returned by this
	 *         context since last call of statsReset(). Undefined if if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the responses2xx.
	 * 
	 * @return Get the number of responses with a 100 status returned by this
	 *         context since last call of statsReset(). Undefined if if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the responses3xx.
	 * 
	 * @return Get the number of responses with a 3xx status returned by this
	 *         context since last call of statsReset(). Undefined if if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the responses4xx.
	 * 
	 * @return Get the number of responses with a 4xx status returned by this
	 *         context since last call of statsReset(). Undefined if if
	 *         setStatsOn(false).
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Gets the responses5xx.
	 * 
	 * @return Get the number of responses with a 5xx status returned by this
	 *         context since last call of statsReset(). Undefined if if
	 *         setStatsOn(false).
	 */
	...
	/**
	 * Gets the encoding by mime type.
	 * 
	 * @param type
	 *            the type
	 * @return the encoding by mime type
	 */
