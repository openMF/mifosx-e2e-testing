...
/* ------------------------------------------------------------ */
/**
 * HTTP Fields. A collection of HTTP header and or Trailer fields. This class is
 * not synchronized and needs to be protected from concurrent access.
 * 
...
public class HttpFields {
	 ...
	 * General Fields.
	 */
	public final static String __CacheControl = "Cache-Control",
			__Connection = "Connection", __Date = "Date", __Pragma = "Pragma",
			__ProxyConnection = "Proxy-Connection", __Trailer = "Trailer",
			__TransferEncoding = "Transfer-Encoding", __Upgrade = "Upgrade",
			__Via = "Via", __Warning = "Warning";

	/* ------------------------------------------------------------ */
	/**
	 * Entity Fields.
	 */
	public final static String __Allow = "Allow",
			...
			__ContentMD5 = "Content-MD5", __ContentRange = "Content-Range",
			__ContentType = "Content-Type", __Expires = "Expires",
	...
	public final static String __Accept = "Accept",
			...
			__Authorization = "Authorization", __Expect = "Expect",
			__Forwarded = "Forwarded", __From = "From", __Host = "Host",
			__IfMatch = "If-Match", __IfModifiedSince = "If-Modified-Since",
			__IfNoneMatch = "If-None-Match", __IfRange = "If-Range",
			__IfUnmodifiedSince = "If-Unmodified-Since",
			__KeepAlive = "keep-alive", __MaxForwards = "Max-Forwards",
			__ProxyAuthorization = "Proxy-Authorization", __Range = "Range",
			__RequestRange = "Request-Range", __Referer = "Referer",
			__TE = "TE", __UserAgent = "User-Agent",
	 ...
	 * Response Fields.
	 */
	public final static String __AcceptRanges = "Accept-Ranges", __Age = "Age",
			__ETag = "ETag", __Location = "Location",
			__ProxyAuthenticate = "Proxy-Authenticate",
			__RetryAfter = "Retry-After", __Server = "Server",
			__ServletEngine = "Servlet-Engine", __Vary = "Vary",
	 ...
	 * Other Fields.
	 */
	public final static String __Cookie = "Cookie", __SetCookie = "Set-Cookie",
			__SetCookie2 = "Set-Cookie2", __MimeVersion = "MIME-Version",
			__Identity = "identity", __SoapAction = "SOAPAction";
	...
	private static final class FieldInfo {
		...
		public boolean equals(Object o) {
			...
			return fi == this || fi._hashCode == _hashCode
	...
	static {
		...
		__values.put("ISO-8859-1, utf-8;q=0.66, *;q=0.66",
				"ISO-8859-1, utf-8;q=0.66, *;q=0.66");
	}

	/* ------------------------------------------------------------ */
	/** The Constant __separators. */
	...
	public static void formatDate(StringBuffer buf, Calendar calendar,
		...
		long tm = (calendar instanceof HttpCal) ? (((HttpCal) calendar)
				.getTimeInMillis()) : calendar.getTime().getTime();
	...
	/** The Constant __dateCache. */
	public final static DateCache __dateCache = new DateCache(
			"EEE, dd MMM yyyy HH:mm:ss 'GMT'", Locale.US);

	/* ------------------------------------------------------------ */
	/** The Constant __dateReceiveFmt. */
	private final static String __dateReceiveFmt[] = {
			...
			"EEE, dd-MMM-yy HH:mm:ss zzz", "dd MMM yyyy HH:mm:ss",
			"dd-MMM-yy HH:mm:ss", };

	/** The __date receive source. */
	public static SimpleDateFormat __dateReceiveSource[];

	/** The Constant __dateReceiveCache. */
	...
	static {
		...
		for (int i = 0; i < __dateReceiveSource.length; i++) {
			__dateReceiveSource[i] = new SimpleDateFormat(__dateReceiveFmt[i],
					Locale.US);
			__dateReceiveSource[i].setTimeZone(__GMT);
		}
	}

	/** The Constant __01Jan1970. */
	...
	private static final class Field {
		...
		/* ------------------------------------------------------------ */
		/**
		 * Reassign a value to this field. Checks if the value is the same as
		 * that in the char array, if so then just reuse existing value.
		 * 
		 * @param buf
		 *            the buf
		 * @param offset
		 ...
		 */
		public String toString() {
			return ("[" + (_prev == null ? "" : "<-") + getDisplayName() + "="
					+ _value + (_next == null ? "" : "->") + "]");
		}
	}
	...
	Field getField(FieldInfo info, boolean getValid) {
		...
		if (hi < _index.length) {
			if (_index[hi] >= 0) {
				...
				return (field != null && (!getValid || field._version == _version)) ? field
						: null;
			}
		} else {
			for (int i = 0; i < _fields.size(); i++) {
				...
				if (info.equals(field._info)
						&& (!getValid || field._version == _version))
					return field;
			}
		}
		return null;
	 ...
	 * @return the value of a field, or null if not found. For multiple fields
	 *         of the same name, only the first is returned.
	 */
		...
		return new Enumeration() {
			...
			public boolean hasMoreElements() {
				...
				while (e.hasMoreElements()) {
					...
					tok = new QuotedStringTokenizer(value, separators, false,
							false);
	 ...
	 *             If the name is a single valued field and already has a value.
	 */
	...
	/* -------------------------------------------------------------- */
	/**
	 * Get a header as an integer value. Returns the value of an integer field
	 * or -1 if not found. The case of the field name is ignored.
	 * 
	 * @param name
	 *            the case-insensitive field name
	 * @return the int field
	 * @throws NumberFormatException
	 *             If bad integer found
	 */
	...
	public long getDateField(String name) {
		...
		for (int i = 0; i < _dateReceive.length; i++) {
			...
			if (_dateReceive[i] == null)
				_dateReceive[i] = (SimpleDateFormat) __dateReceiveSource[i]
						.clone();
	...
	public static String valueParameters(String value, Map parameters) {
		...
		StringTokenizer tok1 = new QuotedStringTokenizer(value.substring(i),
				";", false, true);
	...
	public static Float getQuality(String value) {
		...
		if (value.charAt(qe++) == 'q') {
			qe++;
			Map.Entry entry = __qualities.getEntry(value, qe, value.length()
					- qe);
			if (entry != null)
			...
			if (maxAge >= 0) {
				if (version == 0) {
					...
					if (maxAge == 0)
						...
						formatDate(buf, System.currentTimeMillis() + 1000L
								* maxAge, true);
				} else {
					buf.append(";Max-Age=");
