...
/* ------------------------------------------------------------ */
/**
 * A HTTP Connection. This class provides the generic HTTP handling for a
 * connection to a HTTP server. An instance of HttpConnection is normally
 * created by a HttpListener and then given control in order to run the protocol
 * handling before and after passing a request to the HttpServer of the
 * HttpListener.
 * 
 * This class is not synchronized as it should only ever be known to a single
 * thread.
 * 
 * @see HttpListener
 * @see HttpServer
...
public class HttpConnection implements OutputObserver {
	...
	/* ------------------------------------------------------------ */
	/** The __thread connection. */
	private static ThreadLocal __threadConnection = new ThreadLocal();

	/**
	 * Support for FRC2068 Continues. If true, then 100 Continues will be sent
	 * when expected or for POST requests. If false, 100 Continues will only be
	 * sent if expected. Can be configured with the
	 * org.mortbay.http.HttpConnection.2068Continue system property.
	 */
	private static boolean __2068_Continues = Boolean
			.getBoolean("org.browsermob.proxy.jetty.http.HttpConnection.2068Continue");
	...
	/** The _handling thread. */
	private Thread _handlingThread;

	/** The _remote inet address. */
	 ...
	 * @param in
	 *            InputStream to read request(s) from.
	 * @param out
	 *            OutputputStream to write response(s) to.
	 * @param connection
	 *            The underlying connection object, most likely a socket. This
	 *            is not used by HttpConnection other than to make it available
	 *            via getConnection().
	 */
	public HttpConnection(HttpListener listener, InetAddress remoteAddr,
			InputStream in, OutputStream out, Object connection) {
		...
		_resolveRemoteHost = _listener != null
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the ThreadLocal HttpConnection. The ThreadLocal HttpConnection is set
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the underlying connection object. This opaque object, most likely a
	 * socket. This is not used by HttpConnection other than to make it
	 * available via getConnection().
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Close the connection. This method calls close on the input and output
	 * streams and interrupts any thread in the handle method. may be
	 * specialized to close sockets etc.
	 * 
	 * @throws IOException
	...
	public void close() throws IOException {
		try {
			...
			if (_connection instanceof Socket
					&& !(_connection instanceof SSLSocket))
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the listeners HttpServer. But if the name is 0.0.0.0, then the real
	...
	/* ------------------------------------------------------------ */
	/**
	 * Checks if is throttled.
	 * 
	 * @return True if this connections state has been altered due to low
	 *         resources.
	 */
	 ...
	 * 
	 * @param throttled
	 *            True if this connections state has been altered due to low
	 *            resources.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get associated object. Used by a particular HttpListener implementation
	 * to associate private datastructures with the connection.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set associated object. Used by a particular HttpListener implementation
	 * to associate private datastructures with the connection.
	 * 
	 * @param o
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set a HttpTunnel for the connection. A HTTP tunnel is used if the
	 * connection is to be taken over for non-HTTP communications. An example of
	 * this is the CONNECT method in proxy handling. If a HttpTunnel is set on a
	 * connection, then it's handle method is called bu the next call to
	 * handleNext().
	 * 
	 * @param tunnel
	...
	private void verifyHTTP_1_0() {
		...
		if (!_keepAlive
				&& HttpFields.__KeepAlive.equalsIgnoreCase(_request
						.getField(HttpFields.__ProxyConnection)))
			_keepAlive = true;
	...
	private void verifyHTTP_1_1() throws HttpException, IOException {
		...
		String transfer_coding = _request
				.getField(HttpFields.__TransferEncoding);
		...
		if (transfer_coding != null && transfer_coding.length() > 0) {
			...
			if (transfer_coding.equalsIgnoreCase(HttpFields.__Chunked)
					|| StringUtil.endsWithIgnoreCase(transfer_coding,
							HttpFields.__Chunked))
				_inputStream.setChunking();
			else if (StringUtil.asciiToLowerCase(transfer_coding).indexOf(
					HttpFields.__Chunked) >= 0)
				throw new HttpException(HttpResponse.__400_Bad_Request);
		}
		...
		if (expect != null && expect.length() > 0) {
			if (StringUtil.asciiToLowerCase(expect).equals(
					HttpFields.__ExpectContinue)) {
				_inputStream
						.setExpectContinues(_outputStream.getOutputStream());
			} else
				throw new HttpException(HttpResponse.__417_Expectation_Failed);
		} else if (__2068_Continues
				&& _inputStream.available() <= 0
				&& (HttpRequest.__PUT.equals(_request.getMethod()) || HttpRequest.__POST
						.equals(_request.getMethod()))) {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Output Notifications. Trigger header and/or filters from output stream
	 * observations. Also finalizes method of indicating response content
	 * length. Called as a result of the connection subscribing for
	 * notifications to the HttpOutputStream.
	 * 
	 * @param out
	 *            The output stream observed.
	 * @param action
	 *            The action.
	 * @param ignoredData
	 *            the ignored data
	...
	/* ------------------------------------------------------------ */
	/**
	 * Setup the reponse output stream. Use the current state of the request and
	 * response, to set tranfer parameters such as chunking and content length.
	 * 
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	 */
	...
	protected void commit() throws IOException {
		...
		boolean has_close = HttpFields.__Close.equals(_response
				.getField(HttpFields.__Connection));
		if (!_persistent || _close || _listener != null
				&& (!_listener.isStarted() || _listener.isOutOfResources())) {
			_close = true;
			if (!has_close)
		...
		if (_persistent) {
			switch (_dotVersion) {
			case 1: {
				String transfer_coding = _response
						.getField(HttpFields.__TransferEncoding);
				if (transfer_coding == null
						|| transfer_coding.length() == 0
						|| HttpFields.__Identity
								.equalsIgnoreCase(transfer_coding)) {
					...
					if (!HttpFields.__Chunked.equalsIgnoreCase(transfer_coding)) {
						...
						Enumeration enm = _response.getFieldValues(
						...
						while (enm.hasMoreElements()) {
							...
							if (te == null || !te.contains(coding))
								throw new HttpException(
										HttpResponse.__501_Not_Implemented,
										coding);
						}
					}
				}
	...
	private void exception(Throwable e) {
		try {
			...
			if (e instanceof HttpException) {
				...
				_request.setAttribute("javax.servlet.error.exception_type",
						e.getClass());
	...
	/* ------------------------------------------------------------ */
	/**
	 * Service a Request. This implementation passes the request and response to
	 * the service method of the HttpServer for this connections listener. If no
	 * HttpServer has been associated, the 503 is returned. This method may be
	 * specialized to implement other ways of servicing a request.
	 * 
	 * @param request
	 *            The request
	 * @param response
	...
	/* ------------------------------------------------------------ */
	/**
	 * Handle the connection. Once the connection has been created, this method
	 * is called to handle one or more requests that may be received on the
	 * connection. The method only returns once all requests have been handled,
	 * an error has been returned to the requestor or the connection has been
	 * closed. The handleNext() is called in a loop until it returns false.
	 */
	...
	/* ------------------------------------------------------------ */
	/**
	 * Handle next request off the connection. The service(request,response)
	 * method is called by handle to service each request received on the
	 * connection. If the thread is a PoolThread, the thread is set as inactive
	 ...
	 * 
	 * @return true if the connection is still open and may provide more
	 *         requests.
	 */
	public boolean handleNext() {
		...
		try {
			...
			Enumeration connectionValues = _request.getFieldValues(
					HttpFields.__Connection, HttpFields.__separators);
			...
			if (!_request.isHandled() && _listener.getHttpHandler() != null)
				_listener.getHttpHandler()
						.handle("", null, _request, _response);
			...
			if (_request.getState() != HttpMessage.__MSG_RECEIVED) {
				if (log.isDebugEnabled()) {
					if (log.isTraceEnabled())
						log.trace(LogSupport.EXCEPTION, e);
			...
			int bytes_written = 0;
			int content_length = _response == null ? -1 : _response
					.getIntField(HttpFields.__ContentLength);
