...
public class CGI extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug("CGI: CGI bin accepted - " + _docRoot);
		} catch (IOException e) {
		...
		if (log.isDebugEnabled())
			log.debug("CGI: PATH accepted - " + _path);
		if (_path != null)
			_env.set("PATH", _path);
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		String pathInContext = StringUtil.nonNull(req.getServletPath())
	 ...
	 */
	private void exec(File command, String pathInfo, HttpServletRequest req,
		...
		Process p = dir == null ? Runtime.getRuntime().exec(execCmd,
				env.getEnvArray()) : Runtime.getRuntime().exec(execCmd,
				env.getEnvArray(), dir);
		...
		try {
			...
			if (status == null && redirect != null) {
				// The CGI has set Location and is counting on us to do the
				// redirect.
				// See
	...
	private static class EnvList {
		...
		/**
		 * Set a name/value pair, null values will be treated as an empty
		 * String.
		 * 
		 * @param name
		 *            the name
		...
		public String[] getEnvArray() {
			return (String[]) envMap.values()
					.toArray(new String[envMap.size()]);
		}
	}
}
