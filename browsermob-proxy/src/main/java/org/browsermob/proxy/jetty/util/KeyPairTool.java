 ...
 * 
 * <p>
 * The current keytool lacks the ability to insert a key/cert pair sourced from
 * another tool. This utility fills that gap.
 * 
 * <p>
 * Currently this only works for RSA key/cert pairs.
 * 
 * <p>
 * The inverse operation, exporting a keypair to an external format, has been
...
public class KeyPairTool {
	// Default settings...
	/** The key store file. */
	private File keyStoreFile = new File(System.getProperty("user.home"),
			".keystore");
	...
	/* ------------------------------------------------------------ */
	/**
	 * Load parameters and perform the import command. Catch any exceptions and
	 ...
	 * <p>
	 * Class variables hold the state information required for this operation.
	 * 
	 * @throws IOException
	 *             if there are problems with file IO
	 * @throws GeneralSecurityException
	 *             if there are cryptographic failures.
	 * @throws Exception
	 *             on other exceptions, such as classloading failures.
	 */
	private void importKeyPair() throws IOException, GeneralSecurityException,
			Exception {
		...
		try {
			...
			System.out.println("Will load " + keyStoreType + " keystore: "
					+ keyStoreFile);
		...
		keyStore.setKeyEntry(alias, privateKey, keyPassword.toString()
				.toCharArray(), certChain);

		// Save the KeyStore
		FileOutputStream keyStoreOut = new FileOutputStream(keyStoreFile);
		keyStore.store(keyStoreOut, keyStorePassword.toString().toCharArray());
		keyStoreOut.close();
	 ...
	 * <p>
	 * Note that the certificates must be in the correct order to form a valid
	 * chain starting with the cert corresponding to the private key and ending
	 * with the cert just before the top level cert.
	 * 
	 * @param certFile
	 *            String name of file to load the key from
	 * @return PrivateKey loaded from the file
	 * @throws Exception
	 *             if there are problems with loading the key.
	 */
	private Certificate[] loadCertChain(File certFile) throws Exception {
		...
		try {
			...
			Certificate[] certChain = (Certificate[]) chain
					.toArray(new Certificate[chain.size()]);
	...
	private PrivateKey loadPrivateKey(File privateKeyFile) throws Exception {
		// Load the key file.
		System.out.println("Loading private key from " + privateKeyFile
		...
		// support.
		Class providerClass = Loader.loadClass(this.getClass(),
				providerClassName);
		...
		try {
			...
			System.out.println("Loaded " + privateKey.getAlgorithm() + " "
					+ privateKey.getFormat() + " private key.");
			return privateKey;
		} finally {
	...
	/* ------------------------------------------------------------ */
	/**
	 * Load parameters from the given args and check usage. Will exit on usage
	 * errors.
	 * <p>
	 * Class variables are populated from the command line arguments
	 * 
	 * @param args
	...
	private void loadParameters(String[] args) {
		...
		keyStorePassword = Password.getPassword("jetty.ssl.password", null,
				null);
		keyPassword = Password.getPassword("jetty.ssl.keypassword", null,
