 ...
 * 
 * This is a minimal start to a administration servlet that allows start/stop of
 * server components and control of debug parameters.
 * 
...
public class AdminServlet extends HttpServlet {
	...
	private String doAction(HttpServletRequest request) throws IOException {
		...
		try {
			...
			if (tokens == 1) {
				...
				target += ":" + contextIndex;
				HttpContext context = server.getContext(host, contextPath,
						contextIndex);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		page.add(new Block(Block.Bold).add(new Font(3, true)
				.add(getServletInfo())));
		page.add(Break.rule);
		Form form = new Form(request.getContextPath()
				+ request.getServletPath() + "?A=exit");
		form.method("GET");
		...
		while (s.hasNext()) {
			...
			while (i2.hasNext()) {
				...
				while (i3.hasNext()) {
					...
					java.util.List contextList = (java.util.List) cEntry
							.getValue();
					...
					String id3 = id1
							...
							+ host
							+ ":"
							+ (contextPath.length() > 2 ? contextPath
					...
					for (int i4 = 0; i4 < contextList.size(); i4++) {
						...
						for (int i5 = 0; i5 < handlers; i5++) {
							...
							hItem.add(lifeCycle(request, id5, handler,
									handler.getName()));
	 ...
	 * @return the element
	 */
	private Element lifeCycle(HttpServletRequest request, String id,
			LifeCycle lc) {
	...
	private Element lifeCycle(HttpServletRequest request, String id,
		...
		comp.add("&nbsp;[");
		comp.add(new Link(URI.addPaths(request.getContextPath(),
				request.getServletPath())
				+ "?T="
				+ Long.toString(System.currentTimeMillis(), 36)
				+ "&A="
				+ action + "&ID=" + UrlEncoded.encodeString(id), action));
		comp.add("]");
		return comp;
	}
