...
/**
 * A factory for creating SimulatedSocket objects.
 */
public class SimulatedSocketFactory implements SchemeSocketFactory {
	...
	/**
	 * Instantiates a new simulated socket factory.
	 * 
	 ...
	 *            the stream manager
	 */
	public SimulatedSocketFactory(HostNameResolver hostNameResolver,
	...
	public static <T extends Socket> void configure(T sock) {
		...
		// If we don't set these, we can easily use up too many sockets, even
		// when we're cleaning/closing the sockets
		// responsibly. The reason is that they will stick around in TIME_WAIT
		// for some time (ie: 1-4 minutes) and once
		// they get to 64K (on Linux) or 16K (on Mac) we can't make any more
		// requests. While those limits can be raised
		// with a configuration setting in the OS, we really don't need to
		// change things globally. We just need to make
		// sure that when we close a socket it gets ditched right away and
		// doesn't stick around in TIME_WAIT.
		//
		// This problem is most easily noticable/problematic for load tests that
		// use a single transaction to issue
		// one HTTP request and then end the transaction, thereby shutting down
		// the HTTP socket. This can easily create
		// 64K+ sockets in TIME_WAIT state, preventing any other requests from
		// going out and producing a false-negative
	...
	static {
		try {
			getHostMethod = InetSocketAddress.class.getDeclaredMethod(
					"getHostString", new Class[] {});
	...
	/**
	 * A minor optimization to prevent possible host resolution when inspecting
	 * a InetSocketAddress for a hostname....
	 * 
	 * @param remoteAddress
	 *            the remote address
	 * @return the string
	...
	private String resolveHostName(InetSocketAddress remoteAddress) {
		...
		try {
			hostString = (String) getHostMethod.invoke(remoteAddress,
					new Object[] {});
		} catch (InvocationTargetException ite) {
			throw new RuntimeException(
					"Expecting InetSocketAddress to have a package scoped \"getHostString\" method which returns a String and takes no input");
	...
	public Socket connectSocket(Socket sock, InetSocketAddress remoteAddress,
		...
		if (remoteAddress instanceof HttpInetSocketAddress) {
			hostName = ((HttpInetSocketAddress) remoteAddress).getHttpHost()
					.getHostName();
		} else {
		...
		if (this.hostNameResolver != null) {
			remoteAddr = new InetSocketAddress(
					this.hostNameResolver.resolve(hostName),
