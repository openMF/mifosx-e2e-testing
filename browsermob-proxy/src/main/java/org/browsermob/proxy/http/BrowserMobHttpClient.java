...
public class BrowserMobHttpClient {
	...
	static {
		try {
			...
			LOG.severe(
					"Unable to create User-Agent parser, falling back but proxy is in damaged state and should be restarted",
					e);
			try {
	...
	/**
	 * Sets the user agent parser.
	 * 
	 * @param parser
	 *            the new user agent parser
	 */
	...
	// if captureContent is set, default policy is to capture binary contents
	// too
	/** The capture binary content. */
	...
	// not using CopyOnWriteArray because we're WRITE heavy and it is for READ
	// heavy operations
	...
	/**
	 * Instantiates a new browser mob http client.
	 * 
	 * @param streamManager
	 *            the stream manager
	...
	public BrowserMobHttpClient(StreamManager streamManager,
		...
		httpClientConnMgr = new ThreadSafeClientConnManager(schemeRegistry) {
			...
			public ClientConnectionRequest requestConnection(HttpRoute route,
					Object state) {
				final ClientConnectionRequest wrapped = super
						.requestConnection(route, state);
		...
		// MOB-338: 30 total connections and 6 connections per host matches the
		// behavior in Firefox 3
			...
			protected HttpRequestExecutor createRequestExecutor() {
				return new HttpRequestExecutor() {
					...
					protected HttpResponse doSendRequest(HttpRequest request,
						...
						Date start = new Date();
						HttpResponse response = super.doSendRequest(request,
								conn, context);
					...
					protected HttpResponse doReceiveResponse(
							...
							IOException {
						Date start = new Date();
						HttpResponse response = super.doReceiveResponse(
		...
		httpClient.getParams().setParameter(
				CookieSpecPNames.SINGLE_COOKIE_HEADER, Boolean.TRUE);
		setRetryCount(0);
	 ...
	 * @param proxyRequest
	 *            the proxy request
	 * @return the browser mob http request
	 */
	public BrowserMobHttpRequest newPost(String url,
			org.browsermob.proxy.jetty.http.HttpRequest proxyRequest) {
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpPost(uri), this, -1,
					captureContent, proxyRequest);
	...
	public BrowserMobHttpRequest newGet(String url,
		...
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpGet(uri), this, -1,
					captureContent, proxyRequest);
		} catch (URISyntaxException e) {
	...
	public BrowserMobHttpRequest newPut(String url,
		...
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpPut(uri), this, -1,
					captureContent, proxyRequest);
		} catch (Exception e) {
	...
	public BrowserMobHttpRequest newDelete(String url,
		...
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpDelete(uri), this, -1,
					captureContent, proxyRequest);
		} catch (URISyntaxException e) {
	...
	public BrowserMobHttpRequest newOptions(String url,
		...
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpOptions(uri), this, -1,
					captureContent, proxyRequest);
		} catch (URISyntaxException e) {
	...
	public BrowserMobHttpRequest newHead(String url,
		...
		try {
			URI uri = makeUri(url);
			return new BrowserMobHttpRequest(new HttpHead(uri), this, -1,
					captureContent, proxyRequest);
		} catch (URISyntaxException e) {
	...
	private URI makeUri(String url) throws URISyntaxException {
		...
		// are we using the default ports for http/https? if so, let's rewrite
		// the URI to make sure the :80 or :443
		// is NOT included in the string form the URI. The reason we do this is
		// that in HttpClient 4.0 the Host header
		// would include a value such as "yahoo.com:80" rather than "yahoo.com".
		// Not sure why this happens but we don't
		...
		if ((uri.getPort() == 80 && "http".equals(uri.getScheme()))
			...
			// we rewrite the URL with a StringBuilder (vs passing in the
			// components of the URI) because if we were
			// to pass in these components using the URI's 7-arg constructor
			// query parameters get double escaped (bad!)
	...
	private BrowserMobHttpResponse execute(BrowserMobHttpRequest req, int depth) {
		if (depth >= MAX_REDIRECT) {
			throw new IllegalStateException("Max number of redirects ("
					+ MAX_REDIRECT + ") reached");
		}
		...
		if (har != null && har.getLog().getBrowser() == null) {
			...
			if (uaHeaders != null && uaHeaders.length > 0) {
				...
				try {
					// note: this doesn't work for 'Fandango/4.5.1
					// CFNetwork/548.1.4 Darwin/11.0.0'
					...
					har.getLog().setBrowser(
							new HarNameVersion(browser, version));
				} catch (IOException e) {
					// ignore it, it's fine
				} catch (Exception e) {
		...
		if (os == null) {
			os = new CappedByteArrayOutputStream(1024 * 1024); // MOB-216 don't
																// buffer more
																// than 1 MB
		}
		...
		// link the object up now, before we make the request, so that if we get
		// cut off (ie: favicon.ico request and browser shuts down)
		// we still have the attempt associated, even if we never got a response
		HarEntry entry = new HarEntry(harPageRef);

		// clear out any connection-related information so that it's not stale
		// from previous use of this thread.
		RequestInfo.clear(url, entry);

		entry.setRequest(new HarRequest(method.getMethod(), url, method
				.getProtocolVersion().getProtocol()));
		entry.setResponse(new HarResponse(-999, "NO RESPONSE", method
				.getProtocolVersion().getProtocol()));
		if (this.har != null && harPageRef != null) {
			har.getLog().addEntry(entry);
		}
			...
			for (String k : params.keySet()) {
				for (Object v : params.getValues(k)) {
					entry.getRequest().getQueryString()
							.add(new HarNameValuePair(k, (String) v));
				}
			}
		}
		...
		ActiveRequest activeRequest = new ActiveRequest(method, ctx,
		...
		if (authType == AuthType.NTLM) {
			...
			// ctx.setAttribute("preemptive-auth", new NTLMScheme(new
			// JCIFSEngine()));
		...
		try {
			...
			if (mockResponseCode != -1) {
				...
				// check for null (resp 204 can cause HttpClient to return null,
				// which is what Google does with
				// http://clients1.google.com/generate_204)
				if (is != null) {
					Header contentEncodingHeader = response
							.getFirstHeader("Content-Encoding");
					if (contentEncodingHeader != null
							&& "gzip".equalsIgnoreCase(contentEncodingHeader
									.getValue())) {
						gzipping = true;
					}
			...
			// only log it if we're not shutdown (otherwise, errors that happen
			// during a shutdown can likely be ignored)
			if (!shutdown) {
				LOG.info(String.format("%s when requesting %s", errorMessage,
						url));
			}
		} finally {
		...
		if (captureHeaders || captureContent) {
			for (Header header : method.getAllHeaders()) {
				...
				entry.getRequest()
						.getHeaders()
						.add(new HarNameValuePair(header.getName(), header
								.getValue()));
			}
		...
		if (captureContent) {
			...
			if (method instanceof HttpEntityEnclosingRequestBase
				...
				data.setMimeType(req.getMethod().getFirstHeader("Content-Type")
						.getValue());
				entry.getRequest().setPostData(data);

				if (urlEncoded || URLEncodedUtils.isEncoded(entity)) {
					try {
						final String content = new String(req.getCopy()
								.toByteArray(), "UTF-8");
						if (content != null && content.length() > 0) {
							...
							for (NameValuePair pair : result) {
								params.add(new HarPostDataParam(pair.getName(),
										pair.getValue()));
							}
						}
					} catch (Exception e) {
						LOG.info("Unexpected problem when parsing input copy",
								e);
					}
				} else {
					// not URL encoded, so let's grab the body of the POST and
					// capture that
		...
		javax.servlet.http.Cookie[] cookies = req.getProxyRequest()
				.getCookies();
		...
		if (response != null) {
			try {
				...
				if (contentTypeHdr != null) {
					...
					if (captureContent && os != null
							&& os instanceof ClonedOutputStream) {
						ByteArrayOutputStream copy = ((ClonedOutputStream) os)
								.getOutput();

						if (gzipping) {
							// ok, we need to decompress it before we can put it
							// in the har file
							try {
						...
						if (contentType != null
							...
							entry.getResponse().getContent()
									.setText(new String(copy.toByteArray()));
						} else if (captureBinaryContent) {
							entry.getResponse()
									.getContent()
									.setText(
											Base64.byteArrayToBase64(copy
													.toByteArray()));
						}
					}

					NameValuePair nvp = contentTypeHdr.getElements()[0]
				...
				if (os instanceof ByteArrayOutputStream) {
					responseBody = ((ByteArrayOutputStream) os)
							.toString(charSet);

					if (verificationText != null) {
						contentMatched = responseBody
		...
		if (response != null && statusCode >= 300 && statusCode < 400
				&& statusCode != 304) {
			isRedirect = true;
		...
		// if we didn't mock out the actual response code and the expected code
		// isn't what we saw, we have a problem
		if (mockResponseCode == -1 && expectedStatusCode > -1) {
			if (this.followRedirects) {
				throw new RuntimeException(
						"Response validation cannot be used while following redirects");
			}
			if (expectedStatusCode != statusCode) {
				if (isRedirect) {
					throw new RuntimeException("Expected status code of "
							+ expectedStatusCode + " but saw " + statusCode
		...
		if (isRedirect && (req.getExpectedLocation() != null)) {
			...
			if (location.compareTo(req.getExpectedLocation()) != 0) {
				throw new RuntimeException("Expected a redirect to  "
						+ req.getExpectedLocation() + " but saw " + location);
			}
		}
		...
		return new BrowserMobHttpResponse(entry, method, response,
				contentMatched, verificationText, errorMessage, responseBody,
				contentType, charSet);
	}
	...
	/**
	 * Sets the har page ref.
	 * 
	 * @param harPageRef
	 *            the new har page ref
	 */
	 ...
	 *            the password
	 */
	public void autoNTLMAuthorization(String domain, String username,
			String password) {
	...
	// this method is provided for backwards compatibility before we renamed it
	// to
	...
	class ActiveRequest {
		...
		/**
		 * Instantiates a new active request.
		 * 
		 * @param request
		 *            the request
		 * @param ctx
		...
		void checkTimeout() {
			if (requestTimeout != -1) {
				if (request != null
						&& start != null
						&& new Date(System.currentTimeMillis() - requestTimeout)
								.after(start)) {
					LOG.info(
							"Aborting request to %s after it failed to complete in %d ms",
							request.getURI().toString(), requestTimeout);
		...
		public void abort() {
			...
			// try to close the connection? is this necessary? unclear based on
			// preliminary debugging of HttpClient, but
	...
	private class WhitelistEntry {
		...
		/**
		 * Instantiates a new whitelist entry.
		 * 
		 * @param patterns
		 *            the patterns
		 * @param responseCode
	...
	private class BlacklistEntry {
		...
		/**
		 * Instantiates a new blacklist entry.
		 * 
		 * @param pattern
		 *            the pattern
		 * @param responseCode
	...
	private class RewriteRule {
		...
		/**
		 * Instantiates a new rewrite rule.
		 * 
		 * @param match
		 *            the match
		 * @param replace
	 ...
	 */
	public static long copyWithStats(InputStream is, OutputStream os)
			throws IOException {
		long bytesCopied = 0;
