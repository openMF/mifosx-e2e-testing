...
/**
 * The Class BrowserMobProxyHandler.
 */
public class BrowserMobProxyHandler extends SeleniumProxyHandler {
	...
	/** The Constant HEADER_BUFFER_DEFAULT. */
	private static final int HEADER_BUFFER_DEFAULT = 2;

	/** The jetty server. */
	private Server jettyServer;

	/** The header buffer multiplier. */
	...
	/**
	 * Instantiates a new browser mob proxy handler.
	 */
	public BrowserMobProxyHandler() {
		super(true, "", "", false, false);
		setShutdownLock(new Object());

		// set the tunnel timeout to something larger than the default 30
		// seconds
		// we're doing this because SSL connections taking longer than this
		// timeout
		// will result in a socket connection close that does NOT get handled by
		// the
		// normal socket connection closing reportError(). Further, it has been
		// seen
		// that Firefox will actually retry the connection, causing very strange
		// behavior observed in case
		// http://browsermob.assistly.com/agent/case/27843
		//
		// You can also reproduce it by simply finding some slow loading SSL
		// site
		// that takes greater than 30 seconds to response.
		//
		// Finally, it should be noted that we're setting this timeout to some
		// value
		// that we anticipate will be larger than any reasonable response time
		// of a
		// real world request. We don't set it to -1 because the underlying
		// ProxyHandler
		// will not use it if it's <= 0. We also don't set it to Long.MAX_VALUE
		// because
		// we don't yet know if this will cause a serious resource drain, so
		// we're
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.selenium.SeleniumProxyHandler#handleConnect(java
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.browsermob.proxy.selenium.SeleniumProxyHandler#
	 * wireUpSslWithCyberVilliansCA(java.lang.String,
	 * org.browsermob.proxy.selenium.SeleniumProxyHandler.SslRelay)
	 */
	@Override
	protected void wireUpSslWithCyberVilliansCA(String host,
		...
		if (originalHosts != null && !originalHosts.isEmpty()) {
			if (originalHosts.size() == 1) {
				...
				// Warning: this is NASTY, but people rarely even run across
				// this and those that do are solved by this
				// ok, this really isn't legal in real SSL land, but we'll make
				// an exception and just pretend it's a wildcard
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.selenium.SeleniumProxyHandler#getSslRelayOrCreateNew
	 * (org.browsermob.proxy.jetty.util.URI,
	 * org.browsermob.proxy.jetty.util.InetAddrPort,
	 * org.browsermob.proxy.jetty.http.HttpServer)
	 */
	@Override
	 ...
	 *             the exception
	 */
	private void startRelayWithPortTollerance(HttpServer server,
			SslRelay relay, int tries) throws Exception {
		if (tries >= 5) {
			throw new BindException(
					"Unable to bind to several ports, most recently "
							+ relay.getPort() + ". Giving up");
		}
		try {
			if (server.isStarted()) {
				relay.start();
			...
			LOG.info("Unable to bind to port %d, going to try port %d now",
					relay.getPort(), relay.getPort() + 1);
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.selenium.SeleniumProxyHandler#newHttpTunnel(org.
	 * browsermob.proxy.jetty.http.HttpRequest,
	 * org.browsermob.proxy.jetty.http.HttpResponse, java.net.InetAddress, int,
	 * int)
	 */
	@Override
	protected HttpTunnel newHttpTunnel(HttpRequest httpRequest,
			HttpResponse httpResponse, InetAddress inetAddress, int i, int i1)
			throws IOException {
		// we're opening up a new tunnel, so let's make sure that the associated
		// SslRelay (which may or may not be new) has the proper buffer settings
		adjustListenerBuffers();

		return super.newHttpTunnel(httpRequest, httpResponse, inetAddress, i,
				i1);
	}
	 ...
	 * 
	 * @see
	 * org.browsermob.proxy.selenium.SeleniumProxyHandler#proxyPlainTextRequest
	 * (java.net.URL, java.lang.String, java.lang.String,
	 * org.browsermob.proxy.jetty.http.HttpRequest,
	 * org.browsermob.proxy.jetty.http.HttpResponse)
	 */
	@SuppressWarnings({ "unchecked" })
	protected long proxyPlainTextRequest(final URL url, String pathInContext,
			String pathParams, HttpRequest request, final HttpResponse response)
			throws IOException {
		try {
			String urlStr = url.toString();

			// We don't want selenium-related showing up in the detailed
			// transaction logs
			...
			// we also don't URLs that Firefox always loads on startup showing
			// up, or even wasting bandwidth.
			// so for these we just nuke them right on the spot!
			if (urlStr.startsWith("https://sb-ssl.google.com:443/safebrowsing")
					|| urlStr
							.startsWith("http://en-us.fxfeeds.mozilla.com/en-US/firefox/headlines.xml")
					|| urlStr
							.startsWith("http://fxfeeds.mozilla.com/firefox/headlines.xml")
					|| urlStr
							.startsWith("http://fxfeeds.mozilla.com/en-US/firefox/headlines.xml")
					|| urlStr
							.startsWith("http://newsrss.bbc.co.uk/rss/newsonline_world_edition/front_page/rss.xml")) {
				// don't even xfer these!
				request.setHandled(true);
				return -1;
			}

			// this request must have come in just as we were shutting down,
			// since there is no more associted http client
			// so let's just handle it like we do any other request we don't
			...
			if ("GET".equals(request.getMethod())) {
				...
				LOG.warn("Unexpected request method %s, giving up",
						request.getMethod());
				request.setHandled(true);
				return -1;
			}
			...
			httpReq.setRequestCallback(new RequestCallback() {
				...
				public void handleHeaders(Header[] headers) {
					for (Header header : headers) {
						if (reportHeader(header)) {
							response.addField(header.getName(),
									header.getValue());
						}
					}
				}
				...
				public boolean reportHeader(Header header) {
					// don't pass in things like Transfer-Encoding and other
					// headers that are being masked by the underlying
					// HttpClient impl
					return !_DontProxyHeaders.containsKey(header.getName())
							&& !_ProxyAuthHeaders.containsKey(header.getName());
				}
			...
			// ALWAYS mark the request as handled if we actually handled it.
			// Otherwise, Jetty will think non 2xx responses
			// mean it wasn't actually handled, resulting in totally valid 304
			// Not Modified requests turning in to 404 responses
			// from Jetty. NOT good :(
			request.setHandled(true);
	...
	private static void reportError(Exception e, URL url, HttpResponse response) {
		...
		if (e instanceof UnknownHostException) {
			...
			error = FirefoxErrorContent.NET_INTERRUPT;
				...
				&& e.getMessage().startsWith("Host name may not be null")) {
			error = FirefoxErrorContent.DNS_NOT_FOUND;
		...
		String text = String.format(FirefoxErrorConstants.ERROR_PAGE,
				error.getTitle(), shortDesc, error.getLongDesc());
	...
	/**
	 * Adjust listener buffers.
	 * 
	 * @param headerBufferMultiplier
	 *            the header buffer multiplier
	 */
	public void adjustListenerBuffers(int headerBufferMultiplier) {
		// limit to 10 so there can't be any out of control memory consumption
		// by a rogue script
	...
	public void adjustListenerBuffers() {
		// configure the listeners to have larger buffers. We do this because
		// we've seen cases where the header is
		// too large. Normally this would happen on "meaningless" JS includes
		// for ad networks, but we eventually saw
		// it in a way that caused a Selenium script not to work due to too many
		// headers (see tom.schwenk@musictoday.com)
