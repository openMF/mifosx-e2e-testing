...
public class CertificateCreator {
	...
	/** The Constant clientCertOidsNeverToCopy. */
	private static final HashSet<String> clientCertOidsNeverToCopy = new HashSet<String>();

	/** The Constant clientCertDefaultOidsNotToCopy. */
	...
	/**
	 * X.509 OID for Subject Key Identifier Extension - Replaced when
	 * duplicating a cert.
	 */
	public static final String OID_SUBJECT_KEY_IDENTIFIER = "2.5.29.14";

	/**
	 * X.509 OID for Subject Authority Key Identifier - Replaced when
	 * duplicating a cert.
	 */
	public static final String OID_AUTHORITY_KEY_IDENTIFIER = "2.5.29.35";

	/**
	 * X.509 OID for Issuer Alternative Name - Omitted when duplicating a cert
	 * by default.
	 */
	public static final String OID_ISSUER_ALTERNATIVE_NAME = "2.5.29.8";

	/**
	 * X.509 OID for Issuer Alternative Name 2 - Omitted when duplicating a cert
	 * by default.
	 */
	public static final String OID_ISSUER_ALTERNATIVE_NAME_2 = "2.5.29.18";

	/**
	 * X.509 OID for Certificate Revocation List Distribution Point - Omitted
	 * when duplicating a cert by default.
	 */
	public static final String OID_CRL_DISTRIBUTION_POINT = "2.5.28.31";

	/**
	 * X.509 OID for Authority Information Access - Omitted when duplicating a
	 * cert by default.
	 */
	public static final String OID_AUTHORITY_INFO_ACCESS = "1.3.6.1.5.5.7.1.1";

	/**
	 * X.509 OID for Additional CA Issuers for AIA - Omitted when duplicating a
	 * cert by default.
	 */
	...
	/**
	 * Utility method for generating a "standard" server certificate. Recognized
	 * by most browsers as valid for SSL/TLS. These certificates are generated
	 * de novo, not from a template, so they will not retain the structure of
	 * the original certificate and may not be suitable for applications that
	 * require Extended Validation/High Assurance SSL or other distinct
	 * extensions or EKU.
	 * 
	 * @param newPubKey
	 *            the new pub key
	...
	public static X509Certificate generateStdSSLServerCertificate(
			final PublicKey newPubKey, final X509Certificate caCert,
			final PrivateKey caPrivateKey, final String subject)
			throws CertificateParsingException, SignatureException,
		...
		v3CertGen.setNotAfter(new Date(System.currentTimeMillis() + 30L * 60
				* 60 * 24 * 30 * 12));
		v3CertGen.setNotBefore(new Date(System.currentTimeMillis() - 1000L * 60
				* 60 * 24 * 30 * 12));
		v3CertGen.setIssuerDN(caCert.getSubjectX500Principal());

		// Firefox actually tracks serial numbers within a CA and refuses to
		// validate if it sees duplicates
		// This is not a secure serial number generator, (duh!) but it's good
		// enough for our purposes.
		v3CertGen.setSerialNumber(new BigInteger(Long.toString(System
				.currentTimeMillis())));
		...
		// Firefox 2 disallows these extensions in an SSL server cert. IE7
		// doesn't care.
		...
		// DERObjectIdentifier(OID_ID_AD_CAISSUERS),
		// new GeneralName(GeneralName.uniformResourceIdentifier, "http://" +
		// subject + "/aia")));
	...
	/**
	 * This method creates an X509v3 certificate based on an an existing
	 * certificate. It attempts to create as faithful a copy of the existing
	 * certificate as possible by duplicating all certificate extensions.
	 * 
	 * If you are testing an application that makes use of additional
	 * certificate extensions (e.g. logotype, S/MIME capabilities) this method
	 * will preserve those fields.
	 * 
	 * You may optionally include a set of OIDs not to copy from the original
	 * certificate. The most common reason to do this would be to remove fields
	 * that would cause inconsistency, such as Authority Info Access or Issuer
	 * Alternative Name where these are not defined for the MITM authority
	 * certificate.
	 * 
	 * OIDs 2.5.29.14 : Subject Key Identifier and 2.5.29.35 : Authority Key
	 * Identifier, are never copied, but generated directly based on the input
	 * keys and certificates.
	 * 
	 * You may also optionally include maps of custom extensions which will be
	 * added to or replace extensions with the same OID on the original
	 * certificate for the the MITM certificate.
	 * 
	 * FUTURE WORK: JDK 1.5 is very strict in parsing extensions. In particular,
	 * known extensions that include URIs must parse to valid URIs (including
	 * URL encoding all non-valid URI characters) or the extension will be
	 * rejected and not available to copy to the MITM certificate. Will need to
	 * directly extract these as ASN.1 fields and re-insert (hopefully
	 * BouncyCastle will handle them)
	 * 
	 * @param originalCert
	 *            The original certificate to duplicate.
	 * @param newPubKey
	 *            The new public key for the MITM certificate.
	 * @param caCert
	 *            The certificate of the signing authority fot the MITM
	 *            certificate.
	 * @param caPrivateKey
	 *            The private key of the signing authority.
	 * @param extensionOidsNotToCopy
	 *            An optional list of certificate extension OIDs not to copy to
	 ...
	 *             the no such provider exception
	 */
	public static X509Certificate mitmDuplicateCertificate(
			final X509Certificate originalCert, final PublicKey newPubKey,
			final X509Certificate caCert, final PrivateKey caPrivateKey,
		...
		v3CertGen.setSignatureAlgorithm(CertificateCreator.SIGN_ALGO); // needs
																		// to be
																		// the
																		// same
																		// as
		...
		if (critExts != null) {
			for (String oid : critExts) {
				if (!clientCertOidsNeverToCopy.contains(oid)
					...
					v3CertGen.copyAndAddExtension(new DERObjectIdentifier(oid),
							true, originalCert);
				}
			}
		}
	 ...
	 * 
	 * This method will not add any custom extensions and won't copy the
	 * extensions 2.5.29.8 : Issuer Alternative Name, 2.5.29.18 : Issuer
	 * Alternative Name 2, 2.5.29.31 : CRL Distribution Point or
	...
	public static X509Certificate mitmDuplicateCertificate(
			...
			CertificateNotYetValidException, CertificateException,
			NoSuchAlgorithmException, NoSuchProviderException {
		return mitmDuplicateCertificate(originalCert, newPubKey, caCert,
				caPrivateKey, clientCertDefaultOidsNotToCopy);
	}
	...
	public static X509Certificate createTypicalMasterCert(final KeyPair keyPair)
			throws SignatureException, InvalidKeyException, SecurityException,
		...
		v3CertGen.addExtension(X509Extensions.KeyUsage, false, new KeyUsage(
		...
		typicalCAExtendedKeyUsages.add(new DERObjectIdentifier(
				ExtendedKeyUsageConstants.serverAuth));
		typicalCAExtendedKeyUsages.add(new DERObjectIdentifier(
				ExtendedKeyUsageConstants.OCSPSigning));
		typicalCAExtendedKeyUsages.add(new DERObjectIdentifier(
