 ...
 * record at a time. When directives are seen, they are added to the state and
 * used when parsing future records.
 * 
...
public class Master {
	 ...
	 *            The master file.
	 * @param origin
	 *            The initial origin to append to relative names.
	 * @param ttl
	 *            The initial default TTL.
	 * @throws IOException
	 *             The master file could not be opened.
	 */
	 ...
	 * 
	 * @param in
	 *            The input stream containing a master file.
	 * @param origin
	 *            The initial origin to append to relative names.
	 * @param ttl
	 *            The initial default TTL.
		...
		while (true) {
			...
			if (token.type == Tokenizer.WHITESPACE) {
				...
				if (s.equalsIgnoreCase("$ORIGIN")) {
					...
					/*
					 * If we continued, we wouldn't be looking in the new file.
					 * Recursing works better.
					 */
					return nextRecord();
	...
	/**
	 * Returns an iterator over the generators specified in the master file;
	 * that is, the parsed contents of $GENERATE statements.
	 * 
	 * @return the iterator
	 * @see Generator
	 */
