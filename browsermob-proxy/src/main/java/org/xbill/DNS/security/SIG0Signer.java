...
public class SIG0Signer {
	...
	/**
	 * The default validity period for outgoing SIG(0) signed messages. Can be
	 * overriden by the sig0validity option.
	 */
	 ...
	 * 
	 * @param algorithm
	 *            usually DNSSEC.RSAMD5, DNSSEC.DSA, or DNSSEC.RSASHA1
	 * @param privateKey
	 *            signing key (must match algorithm)
	 * @param name
	 *            the name of the key
	 * @param keyFootprint
	 *            the key tag
	...
	/**
	 * Creates a new SIG(0) signer object. This is the same as the other
	...
	public SIG0Signer(int algorithm, PrivateKey privateKey, Name name,
		...
		Record rec = KEYConverter.buildRecord(name, Type.KEY, DClass.IN, 0,
				KEYRecord.OWNER_USER, KEYRecord.PROTOCOL_ANY, algorithm,
				publicKey);
	...
	public void apply(Message m, byte[] old) throws IOException,
		...
		SIGRecord tmpsig = new SIGRecord(Name.root, DClass.ANY, 0, 0,
				algorithm, 0, timeExpires, timeSigned, footprint, name, null);
		...
		/*
		 * RSA signatures are already in correct format, but Java DSA routines
		 * use ASN.1; convert this to SIG format.
		 */
		if (algorithm == DNSSEC.DSA) {
		...
		SIGRecord sig = new SIGRecord(Name.root, DClass.ANY, 0, 0, algorithm,
				0, timeExpires, timeSigned, footprint, name, signature);
