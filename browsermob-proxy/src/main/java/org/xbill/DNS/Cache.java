 ...
 * validity period is complete. Negative answers are cached, to avoid repeated
...
public class Cache {
	...
	private static class CacheRRset extends RRset implements Element {
		...
		/**
		 * Instantiates a new cache r rset.
		 * 
		 * @param rec
		 *            the rec
		 * @param cred
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.xbill.DNS.Cache.Element#expired()
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.xbill.DNS.Cache.Element#compareCredibility(int)
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.xbill.DNS.RRset#toString()
		 */
	...
	private static class NegativeElement implements Element {
		...
		/**
		 * Instantiates a new negative element.
		 * 
		 * @param name
		 *            the name
		 * @param type
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.xbill.DNS.Cache.Element#getType()
		 */
		public int getType() {
			return type;
		}
	...
	private static class CacheMap extends LinkedHashMap {
		...
		void setMaxSize(int maxsize) {
			/*
			 * Note that this doesn't shrink the size of the map if the maximum
			 * size is lowered, but it should shrink as entries expire.
			 */
			this.maxsize = maxsize;
		}
	...
	/** The Constant defaultMaxEntries. */
	 ...
	 * @return the element
	 */
	private synchronized Element oneElement(Name name, Object types, int type,
			int minCred) {
		Element found = null;
	 ...
	 * @param o
	 *            The source of the record (this could be a Message, for
	 *            example)
	 * @see Record
	 */
	 ...
	 * 
	 * @param name
	 *            The name of the negative entry
	 * @param type
	 *            The type of the negative entry
	 * @param soa
	 *            The SOA record to add to the negative cache entry, or null.
	 *            The negative cache ttl is derived from the SOA.
	 * @param cred
	 *            The credibility of the negative entry
	 */
	public synchronized void addNegative(Name name, int type, SOARecord soa,
			int cred) {
		long ttl = 0;
		if (soa != null)
			ttl = soa.getTTL();
	 ...
	 *         lookup, or null if nothing useful could be cached from the
	 *         message.
	 * @see Message
	 */
	 ...
	 * in this Cache. A negative value disables this feature (that is, sets no
	 * limit).
	 * 
	 * @param seconds
	 *            the new max n cache
	...
	/**
	 * Gets the maximum length of time that records will be stored in this
	 * Cache. A negative value indicates no limit.
	 * 
	 * @return the max cache
	 */
	 ...
	 * infinite limit.
	 * 
	 * Note that setting this to a value lower than the current number of
	 * entries will not cause the Cache to shrink immediately.
	 * 
